...
class FrontPanel(Debugger,QMainWindow,QWidget,ObserverWidget,object):
  ...
  def __init__(self,instrument,parent=None,**kwargs):
    ...
    restoreStateCommand=myMenu.addAction("Load and restore instrument state...")
  ...
  def saveStateInFile(self):
    self.debugPrint('in frontpanel.saveStateInFile() of instrument ',self.instrument.name())
    filename = QFileDialog.getSaveFileName(filter = "instrumentState (*.inst)")
    if filename != '':
      self.instrument.saveStateInFile(filename)
      #self._manager.saveStateAs(filename=filename,instruments=[self.instrument.name()]) # Bad programming

  def restoreStateFromFile(self):
    self.debugPrint('in frontpanel.restoreState() of instrument ',self.instrument.name())
    ...
    if filename != '':
      ...
      # configuration requested from a frontpanel should lead to its Gui update => call updateGui  
