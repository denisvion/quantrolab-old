...
class Instrument(Debugger,ThreadedDispatcher,Reloadable,object):
  ...
  def __init__(self,name=''): # One should always pass a name to create an instrument
    ...
    self._name = name       # the only name of the instrument that the instrument knows.
  ...
  def saveState(self,stateName=None):
    ...
    """
    if stateName is None: stateName='state_'+str(len(self._states))
...
class CompositeInstrument(Instrument):
  ...
  def __init__(self,name,instrumentManager=None,childrenDict=None):
    ...
    self._instrumentManager=instrumentManager  # a composite instrument should not instantiate an intrument manager but use an existing one passed to it.
  ...
  def addChild(self,name,instrument=None,baseClass=None):
    ...
    if name in self._children: 
      ...
      try:
        if baseClass is None: baseClass=name
        module = __import__(baseClass,globals(),globals(),["Instr"],0) # debug here until a module in the smae directory is found
...
import pickle,cPickle  # implements an algorithm for turning an arbitrary Python object into a series of bytes (or chars) or vice and versa.
...
class RemoteInstrument(Debugger,ThreadedDispatcher,Reloadable,object):
  ...
  def __init__(self,name,server,baseclass = None, args = [],kwargs = {},forceReload = False):
    ...
    # server is a ServerConnection (indicated by the instrument manager) that '_send's methods of a single Instrument accross the server
    server.initInstrument(name,baseclass,args,kwargs,forceReload) # initialize the serverConnection with a particular instrument
  ...
  def dispatch(self,command,*args,**kwargs): 
    ...
    # Here we directly go from dispatch to remoteDispatch by overrinding dispatch
    """
    RemoteInstrument overriden 'dispatch' method of original 'ThreadedDispatcher'.
    Aliases 'remoteDispatch'.
    """
    self.debugPrint('in remoteInstrument.dispatch() with command = ',command,' args=',args,' and kwargs=',kwargs)
  ...
  def remoteDispatch(self,command,*args,**kwargs):
    ...
    """
    self.debugPrint('remoteInstrument.remoteDispatch() sending command = ',command,' to ',self._name,'with args=',args,' and kwargs=',kwargs)
    result =  self._server.dispatch(self._name,command,*args,**kwargs) 
    self.debugPrint('remoteDispatch notifying ',command, result)
    self.notify(command,result) # once the result is sent back, we notify the command and its result to all observers of this remote instrument.
    return result
