...
class SmartLoop(Debugger,Subject,Observer,Reloadable):
  ...
  def __init__(self,start=0,step=1,stop=None,linnsteps=None,name='unamed',parent=None,toLoopManager=True,autoreverse=False,autoloop=False,deleteWhenFinished=True):
    ...
    self._name=name
    self._start0=start            # memorize the initial start value in case it is erased by user
    self._stop=stop
    self._step=step
    ...
    if linnsteps!=None:           # specifies a number of equidistant steps up to stop or start + linnsteps steps.
      if stop!=None:
    ...
    self.debugPrint('exiting loop',self.getName(),'.init with start, stop, step = ',self._start,self._stop,self._step)
  ...
  def reinit(self):
    ...
    self._index=None              # index, value and previous value start at None.
    ...
    self._pauseInLast=False       # indicates whether pause was activated during last iteration
    ...
    self._reinitAtTerminate=True  # Will be set to False if a stop is requested so that the loop can be resumed
  ...
  def addChild(self,child):
    ...
    self._children.append(child)                  # append the new child to the children list   
    child.setParent(self)                         # set self as the parent (avoid infinite recursion)
  ...
  def setParent(self,parent):
    ...
    if parent is not None:
      parent.addChild(self)                       # call the addChild (avoid infinite recursion)
  ...
  # Methods for incrementing the loop, determining if it is finished, and for terminating.
  ...
  def next(self):
    ...
    self.increment()                   # increment will call finishedOrNext() and will increment only if not finished
    self.timeUpdate()                  # update timing information
    if self._finished:               
      self.terminate()
      raise StopIteration
    self.updateLoopManager()      
    self._history.append((self._index,self._value,self._time)) # append  (index,output value,time) to the history
    return self._value
  ...
  def increment(self):
    ...
    #  self._imposedValue=None
      ...
      nextVal = self.finishedOrNext() # gets the next value to be applied or None if loop is finished
      if nextVal is None:
        self._finished=True
      else :
        self.incrementIndexValue(nextVal)
    self._imposedValue=None
    self.debugPrint(' new value=',self._value,' and new index=',self._index)
  ...
  def addStep(self):
    ...
    if all([isinstance(v, (int, long)) for v in [val,step] ]): # preserve the integer type
      ...
      return round(val+step,digits2) # avoid accumulating numerical errors due to finite real precision
    else:
      return self._start0
  ...
  def finishedOrNext(self):
    ...
    if self._imposedValue is not None: # try the imposed value as the next one
      ...
      next=self.addStep()              # calculate the next value by adding step to current value
    self.debugPrint('in',self.getName(),'.finishedOrNext with start,stop,previous,val,step,next',self._start,self._stop,self._previousValue,self._value,self._step,next)
    if next is not None:
      startIsNumber,stopIsNumber,previousIsNumber=[isinstance(v, (int, long, float)) for v in[start,stop,previous]] 
      finished=False
      crossStart,crossStop,outside,end=[False]*4
      if previousIsNumber:
        if startIsNumber:
          crossStart= (next>start>=val>=previous) or (next<start<=val<=previous)
        if stopIsNumber:
          crossStop= (next>stop>=val>=previous) or (next<stop<=val<=previous)
      ...
      self.debugPrint('crossStart=',crossStart,' crossStop=',crossStop, 'outside=',outside)
      if crossStart or crossStop or outside:
        ...
        if self._autoreverse:
          ...
          outside2 = False
          if stopIsNumber and startIsNumber:
            outside2 = next - max(start,stop) > epsilon  or min(start,stop) - next > epsilon
          if not outside2:
            finished=False # not finished except if after reversing the step, next value would be outside the range
          self.debugPrint('next=',next,' start=',start,' stop=',stop,' outside2=',outside2)
        elif self._autoloop:
          if crossStart:
            next=self._stop
          else:
  ...
  def getSteps2Go(self,nextVal,mode='normal'):
    steps2Go='inf'
    if nextVal is None:                                                       # nextVal is None only when a loop is finished
      steps2Go=0                                                              # It is self._start at start with self._start being always a number at start
      ...
      if self._value is not None and isinstance(self._start,(int, long, float)):
        steps2Start=int(round((float(self._start)-nextVal)/self._step,0))     # number of intervals at next iteration
        if steps2Start >=0 :
          steps2Go=steps2Start
      if isinstance(self._stop,(int, long,float)):
        steps2Stop=int(round((float(self._stop)-nextVal)/self._step,0))
        if steps2Stop >=0 :
          if steps2Go is None or steps2Stop < steps2Go:
            steps2Go = steps2Stop
      if isinstance(steps2Go,(int,long)):
        steps2Go+=1                                                           # number of intervals at next iteration +1 + current steps
    return steps2Go
  ...
  def setStart(self,newStart):
    ...
    if newStart is not None:
      self._start0=newStart   # memorize the last start different from None to be able to restart a loop
...
class PredefinedLoop(SmartLoop):
  ...
  def __init__(self,start=0,values=range(10),**kwargs):
    self._values=values
    SmartLoop.__init__(self,**kwargs)                       # initialize start and index
    if not isinstance(self._stop,(int,long)) or self._stop<0 or self._stop>len(values)-1:
      self._stop=len(values)-1                              # and stop as the last index if not already defined 
    self._previousIndex=None

  def next(self):
    return self._values[super(PredefinedLoop, self).next()]  # the value has the index in a predefined loop

  def getValue(self):
    return self._values[self._value]                        # the value has the index in a predefined loop
...
class AdaptiveLoop(SmartLoop):
  ...
  def preIncrement(self): # overidden method of the SmartLoop parent class
    self.debugPrint('in preincrement calling self._adaptFunc with feedBackValues=',self._feedBackValues)
