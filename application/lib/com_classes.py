...
class Dispatcher(Subject):
    ...
    def dispatch(self, command, *args, **kwargs):
      ...
      """
      self.queue.insert(0,[self._currentId,command,None,args,kwargs])     # insert message in queue with callback=None
      self._currentId+=1                                                  # increment message index
      self._stopDispatcher = False                                        # and enable processing

    def dispatchCB(self, command, callback, *args, **kwargs):
      """
      Dispatcher method that dispatches a command with callback.
      """
      self.queue.insert(0,[self._currentId,command,callback,args,kwargs]) # insert message in queue with a callback
      self._currentId+=1                                                  # increment message index
      self._stopDispatcher = False                                        # and enable processing
    ...
    def processQueue(self):
      ...
      if len(self.queue) == 0:                                           # queue is empty => return
        return
      while len(self.queue) > 0:
        (dispatchId,command,callback,args,kwargs) = self.queue.pop()     # pop a message from the queue
        mname = command
        if hasattr(self, mname):                                         # if the method name specified in the message is an existing method
            method = getattr(self, mname)
            if DEBUG: print mname,method,args,kwargs
            result = method(*args,**kwargs)                              # run it and get the result
            self.notify(command,result)                                  # notify the name of the method run and its result (this is different from callback)
            if callback != None:
              callback(dispatchId,result)                                # this is the callback call
        else:
            self.error()                                                 # the method name does not correspond to an existing method => error
      elapsed = (time.time() - start)                                    # calculate total duration of queue processing
