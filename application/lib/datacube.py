...
import sys,yaml,string,StringIO,os,os.path,pickle,copy,time,weakref,re
...
class Datacube(Subject,Observer,Reloadable,Debugger):
  ...
  def setParameters(self,paramDict):
    ...
    self.setModified()
    self.debugPrint('datacube.setParameters with datacube ',self.name(),' notifying ""parameters"" with parameters=',paramDict)
  ...
  def addParameters(self,paramDict):
    ...
    """
    for key in paramDict.keys(): self._parameters[key]=paramDict[key]
    self.setModified()
    self.debugPrint('datacube.addParameters with datacube ',self.name(),' notifying ""parameters"" with parameters=',paramDict)
    self.notify("parameters",self._parameters)
    self.notify("metaUpdated",self._meta)
  ...
  def _adjustTable(self,rowIndex=None,notifyFields=True,reserve=0):
    ...
    #else:
    if rowIndex is None : rowIndex=self._meta["length"]-1
    ...
    if unchangedCols and ta is not None :                   # if fields (colum names) have'nt changed
      ...
      if ta is not None : nbrRows=max(nbrRows,len(ta))
  ...
  def renameColumn(self,oldName,newName):
    ...
    if newName is None: newName=self.newColumnName()
    if oldName in fN:
  ...
  def _addFields(self,nameIndexDict,adjustTable=True):
    ...
    for name in sortedNames:
      ...
      if name is None: name=self.newColumnName()                                  
  ...
  def createCol(self,name=None,columnIndex=None,offsetRow=0,values=None,notify=True,**kwargs):
    ...
    if notify:
      self.notify("names",self._meta["fieldNames"])
      if values is not None: self.notify("commit")
  ...
  def insertRow(self,rowIndex=None,before=True,notify=False,commit=False,**keys):
    ...
    if rowIndex is None: index=self._meta["index"]
  ...
  def extendTo(self,rowIndex=None,reserve=0,extendLength=False):
    ...
    if rowIndex is None: rowIndex=self._meta["index"]
  ...
  def set(self,rowIndex=None,notify=False,commit=False,columnOrder=None,extendLength=False,**keys):
    ...
    if columnOrder:                                        # Put in it first the keys of columnOrder (even if no specified value) if not already existing
      for key in columnOrder :
        if key not in existingKeys: specifiedKeys.append(key)
    ...
    if rowIndex is None : rowIndex = self._meta["index"]     # defines the row index for the set
  ...
  def saveTable(self,filename,delimiter = "\t",header = None):
    ...
    if header != None:  file.write(header)
  ...
  def savetxt(self,path = None,saveChildren = True,overwrite = False,forceSave = False,newFile=True,header = False, folders=False):
    ...
    for i in range(0,le):
      ...
      if childname is None: childname='child'
    ...
    if os.path.exists(savepath) and (os.path.exists(parpath) or header):
        self.debugPrint('unsaved, par file, txt file and cube modification times =',self._unsaved,' ,  ',os.path.getmtime(savepath),' , ',float(self._meta["modificationTime"]))
  ...
  def loadtxt(self,path,format = 'yaml',loadChildren = True):
    ...
    if os.path.exists(path+".par"):       # try first to load parameters from filename.par
      ...
      content=file.read()
      file.close()
      data=yaml.load(content.split('#end of header\n')[0])          # retrieve the yaml by cutting just before the '/n#end of header/n'.
