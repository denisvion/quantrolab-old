...
class Datacube(Subject, Observer, Reloadable, Debugger):
    ...
    Predefined properties:
      A datacube has predefined properties stored in a meta dictionary, such as
      ...
      - parameters: a dictionary of free parameters available to uesers (can be used for instance to store the parameters used to build the data)
    ...
    def initialize(self, name="cube", description="", filename=None, dtype=float64, defaults=None):
        ...
        # ordered list of field (column) names
        self._meta["fieldNames"] = []
        self._meta["description"] = description
        # dictionary of the field names with values equal to their index
        self._meta["fieldMap"] = dict()
        # can appear as redundant but make easier the access by name and
        # provides a memory on column creation or supression.
    ...
    def setName(self, name):
        ...
        self.debugPrint('datacube.setName with datacube ',
                        self.name(), ' notifying "name with name=', name)
    ...
    def setFilename(self, filename):
        ...
        self.setModified()
        self.debugPrint('datacube.setFilename with datacube=',
                        self, ' notifying ""filename"" with filename=', filename)
    ...
    def setTags(self, tags):
        ...
        self.setModified()
        self.debugPrint('datacube.setTags with datacube ',
                        self.name(), ' notifying ""tags"" with tags=', tags)
    ...
    def structure(self, tabs=""):
        ...
        """
        string = tabs + \
            "cube(%d,%d)" % (self._meta["length"],
                             len(self._meta["fieldNames"])) + "\n"
    ...
    def names(self, includeChildren=False, upToLevel=-1, flatten=True):
        ...
        names = list(self._meta[
                     "fieldNames"])  # Very important: clone the list _meta["fieldNames"] to protect it from modification outside
        if not includeChildren:
            ...
            if upToLevel == 0 or not self.children():
                ...
                upToLevel -= 1
                new = [child.names(includeChildren=includeChildren, upToLevel=upToLevel,
                                   flatten=flatten) for child in self.children()]
            if flatten:
        ...
        while hasChildren:              # loop while there are still children everywhere at the next level
            ...
            for child in children:
                ...
                if not newCommonNames:
                    ...
                    # but return the first time there are no common names at
                    # the current level
                    return commonNames
                else:
                    newChildren = child.children()
    ...
    def _resize(self, size):
        ...
        """
        self._table.resize(
            size, refcheck=False)  # size is a tuple (nbrRows,nbrColumns)
    ...
    def _adjustTable(self, rowIndex=None, notifyFields=True, reserve=0):
        ...
        """
        self.debugPrint('In ', self._meta[
                        "name"], '._adjustTable(rowIndex=', rowIndex, ',reserve=', reserve, ')')
        fieldNames, fieldMap, ta = self._meta[
            "fieldNames"], self._meta["fieldMap"], self._table
        ...
        unchangedCols = len(fieldNames) == len(fieldMap) and all([fieldNames[fieldMap[
            field]] == field for field in fieldMap])  # true also for empty fieldNames and fieldMap
        # if fields (colum names) have'nt changed
        if unchangedCols and ta is not None:
            if rowIndex >= len(self._table):  # => adjust only if room is missing
                # => simple resizing without any copy
                self._resize((nbrRows, nbrCols))
        else:                                             # else if column have changed
            if ta is not None:
                nbrRows = max(nbrRows, len(ta))
            newarray = zeros((nbrRows, nbrCols), dtype=self._meta[
                             "dataType"])  # create and new array
            # and if the table already exist begin to copy the old data in the
            # new array:
            if ta is not None:
                for i in range(nbrCols):  # copy column by column the old columns
                    # determine if the column i exists in the previous fieldMap
                    if fieldNames[i] in fieldMap:
                        j = fieldMap[fieldNames[i]]
                        # copy from the old to new array
                        newarray[:len(ta), i] = ta[:, j]
            self._table = newarray                          # update table with new array
        # update the fieldMap. It is now again in agreement with the fieldName
        # list.
        self.updateFieldMap()
        if notifyFields:
    ...
    def removeColumns(self, namesOrIndices, notify=True):
        ...
        self.setModified()
        self.debugPrint('In ', self._meta[
                        "name"], '.removeColumns(namesOrIndices=', namesOrIndices, ',notify=', notify, ')')
        names = []
        ...
        for name in names:
            if name in self._meta["fieldNames"]:
                del self._meta["fieldNames"][
                    self._meta["fieldNames"].index(name)]
    ...
    def _addFields(self, nameIndexDict, adjustTable=True):
        ...
        self.setModified()
        self.debugPrint('In ', self._meta[
                        "name"], '_addFields(nameIndexDict=', nameIndexDict, ',adjustTable=', adjustTable, ')')
        newField = False
        ...
        for name in sortedNames:
            ...
            if existingColIndex is None:                                # if field already present don't do anything
                # name not in fieldNames=> new names to notify soon
                newField = True
                nbrCols = len(self._meta["fieldNames"])
                if colIndex is not None and colIndex < 0:                       # an index was indicated
                    # realculate its positive value if negative (relative to
                    # the end)
                    colIndex = nbrCols + colIndex + 1
                    if colIndex < 0:
                        colIndex = 0                             # if still negative insert at index 0
                if colIndex is None or colIndex > nbrCols:               # if name and index not given or index too large
                    # insert at index length (first free index)
                    colIndex = nbrCols
                # insert the name in fieldNames
                self._meta["fieldNames"].insert(colIndex, name)
            else:
                colIndex = existingColIndex
        if adjustTable:
            # New fieldNames now contradicts fieldmap and table => call
            # _adjustTable
            self._adjustTable()
    ...
    def createCol(self, name=None, columnIndex=None, offsetRow=0, values=None, notify=True, **kwargs):
        ...
        # Update fieldNames but wait before adjusting the table that we know
        # adapt the length to the passed rows
        newField, columnIndex = self._addFields(
            {name: columnIndex}, adjustTable=False)
        if values is not None:
            if offsetRow < 0:
                # offsetRow is the index where to start to write
                offsetRow = self._meta["length"] + offsetRow + 1
            if offsetRow < 0:
                offsetRow = 0
            # maxRow is the final length
            maxRow = offsetRow + len(values)
    ...
    def removeRow(self, row, notify=False):
        ...
        if self._meta["index"] >= row:
            self._meta["index"] -= 1
        self.debugPrint('datacube.removeRow with datacube ',
                        self.name(), ' notifying ""removeRow"" with row=', row)
        if notify:
    ...
    def removeRows(self, rows, notify=False):
        ...
        if rows and len(rows) != 0:
            # important to reverse for sequential removing
            sortedRows = reversed(sorted(rows))
    ...
    def insertRow(self, rowIndex=None, before=True, notify=False, commit=False, **keys):
        ...
        self.debugPrint(self._meta["name"], '.insertRow(rowIndex=', rowIndex,
                        ',before=', before, ',notify=', notify, ',commit=', commit, str1, ')')
        ...
        if index < self._meta["length"]:
            # extend datacube table if needed
            self.extendTo(rowIndex=self._meta["length"])
            # copy and paste one row below
            self._table[index + 1:, :] = self._table[index:-1, :]
        # call set without propagating notify and commit (managed directly
        # below)
        self.set(rowIndex=index, **keys)
        if oldIndex >= index:
            # by default the data pointed to by index are the same as before
            self._meta["index"] += 1
        if commit:
            # if commit is forced, current row becomas index+1 (dangerous)
            self.commit(rowIndex=index)
        elif notify:
    ...
    def insertRows(self, rowIndex=None, before=True, notify=False, commit=False, numberOfRows=1, **keys):
        ...
        for i in range(numberOfRows):
            self.insertRow(rowIndex=rowIndex + i, before=before,
                           notify=False, commit=False, **keys)
        if commit:
    ...
    def extendTo(self, rowIndex=None, reserve=0, extendLength=False):
        ...
        """
        self.debugPrint('In ', self._meta[
                        "name"], 'extendTo(rowIndex=', rowIndex, ',reserve=', reserve, ',extendLength=', extendLength, ')')
        if rowIndex is None:
            rowIndex = self._meta["index"]
        self._adjustTable(rowIndex=rowIndex, reserve=reserve,
                          notifyFields=False)
    ...
    def set(self, rowIndex=None, notify=False, commit=False, columnOrder=None, extendLength=False, **keys):
        ...
        Important: This function does not change the datacube's current row index unless commit is explicitely set to true.
          WARNING: changing the datacube's current row index is dangerous if several callers edit the datacube simult  aneously.
        """
        str1 = ''
        for key in keys:
            str1 = str1 + ',' + key + "=" + str(keys[key])
        ...
        # Build a list of fields (column names) to be added
        specifiedKeys = []
        # Put in it first the keys of columnOrder (even if no specified value)
        ...
        for key in specifiedKeys:
            # all fields to be inserted one by one at the end
            nameIndexDict[key] = i
            i += 1
        # then call _addFields to add the new fields (column names) but still
        # wait before adjustTabl
        newFields, colIndex = self._addFields(nameIndexDict, adjustTable=False)
        if rowIndex is None:
            # defines the row index for the set
            rowIndex = self._meta["index"]
        elif rowIndex < 0:
        ...
        self.extendTo(rowIndex=rowIndex, reserve=500,
                      extendLength=extendLength)
        newData = False
        for key in keys:                                        # for each key
            newData = True
            columnIndex = self._meta["fieldMap"][key]
            # add the corresponding value in the table
            self._table[rowIndex, columnIndex] = keys[key]
        if newFields:
            self.debugPrint('datacube ', self.name(),
                            'notifying "names"=', self._meta["fieldNames"])
            # send only one notification if new names have been added
            self.notify("names", self._meta["fieldNames"])
        if newData:
            if commit:
                # and commit if requested
                self.commit(rowIndex=rowIndex)
            elif notify:
                self.debugPrint('datacube ', self.name(),
                                'notifying "commit" with index=', rowIndex)
    ...
    def commit(self, rowIndex=None, gotoNextRow=True):
        ...
        """
        self.debugPrint('In', self._meta[
                        "name"], '.commit (rowIndex=', rowIndex, ',gotoNextRow=', gotoNextRow, ')')
        if rowIndex is None:
            rowIndex = self._meta["index"]
        if rowIndex >= self._meta["length"]:
            self.extendTo(rowIndex=rowIndex, extendLength=True)
        if gotoNextRow:
            # possibly 1st row outside datacube
            self._meta["index"] = rowIndex + 1
        self.notify("commit", rowIndex)
        self._unsaved = True
    ...
    def sortBy(self, column, reverse=False):
        ...
        self.debugPrint('datacube.sortBy with datacube ',
                        self.name(), ' notifying "sortBy" with column=', column)
    ...
    def removeChild(self, childCube, deleteChildCube=False):
        ...
        for child in self._children:
            if child.datacube() == childCube:
                ...
                # remove child from memory (does not delete the datacube
                # itself)
                del child
                # notify of the deletion
                self.notify("removeChild", childCube)
                if deleteChildCube:                          # if child cube and its descendents to be deleted
                    for child2 in childCube._children:         # call recursively the function to delete descendents
                        childCube.removeChild(
                            child2._datacube, deleteChildCube=deleteChildCube)
    ...
    def addChild(self, childCube, **kwargs):
        ...
        """
        self.debugPrint('in ', self.name(),
                        '.addChild (childCube) with childCube =', childCube)
        if childCube == self:
        ...
        self.debugPrint('datacube.addChild with datacube ', self.name(
        ), ' notifying "addChild" with cube=', childCube)
    ...
    def attributesOfChildren(self, common=False):
        ...
        if len(childrenCubes) > 0:
            attributeKeys = set(self.attributesOfChild(
                childrenCubes[0]).keys())  # initialize
    ...
    def attributesOfChild(self, childCube):
        ...
        if childCube in childrenCubes:
            ...
            # return a copy (dict) to protect the attributes
            return dict(self._children[i].attributes())
    ...
    def loadTable(self, filename, delimiter="\t", guessStructure=False):
        ...
        contents = file.read()
        file.close()
        # eliminate a possible header (added by DV in Jan 2015)
        ...
        self._table = zeros((len(lines[1:]), len(
            self._meta["fieldNames"])), dtype=self._meta["dataType"])
        self._meta["length"] = 0
        i = 0
        for line in lines[1:]:
    ...
    def saveToHdf5(self, path=None, saveChildren=True, overwrite=False, forceSave=False, verbose=False):
        ...
        self.saveToHdf5Object(dataFile, saveChildren,
                              overwrite, forceSave, verbose=verbose)
    ...
    def savetxt(self, path=None, saveChildren=True, overwrite=False, forceSave=False, newFile=True, header=False, folders=False):
        ...
        if not overwrite:
            if os.path.exists(directory + "/" + basename + ".txt"):
                if newFile:
                    ...
                    print "FILE(s) NOT SAVED: Cannot overwrite existing files or create new ones. Modify overwrite and/or newFile keyword arguments."
                    return
        ...
        for i in range(0, le):
            ...
            self.debugPrint(
                'calling recursively savetxt for child', childfilename)
            ...
            children.append(
                {'attributes': item.attributes(), 'path': childPath})
        ...
        if save or forceSave:
            ...
            # the dictionary is serialized using YAML
    ...
    def loadtxt(self, path, format='yaml', loadChildren=True):
        ...
        """
        path = re.sub(r"\.[\w]{3}$", "",
                      path)  # eliminate the suffix .txt or .par
        filename = os.path.split(path)[1]
        directory = os.path.abspath(os.path.dirname(path))
        # try first to load parameters from filename.par
        if os.path.exists(path + ".par"):
            params = open(path + ".par", "r")
            data = yaml.load(params.read())
            params.close()
            self.setFilename(directory + "/" + filename + ".par")
        # and then from the header in filename.txt (if filename.par not found)
        elif os.path.exists(path + ".txt"):
            file = open(path + ".txt", "r")
            content = file.read()
            file.close()
            # retrieve the yaml by cutting just before the '/n#end of
            # header/n'.
        ...
        if version in ["0.1", "0.2", "0.3", "0.4"]:
            ...
            mapping = {"len": "length", "index": "index", "names": "fieldNames",
                       "name": "name", "description": "description", "tags": "tags", "dtype": "dataType"}
    ...
    def plot(self, fig=None, x=None, y=None, ls='-', marker='o', color='b', **kwargs):
        ...
        def filter(t):
            if t and isinstance(t, basestring):
                if t in names:
                    # replace a string that is not an existing name by None
                    t = [t]
                else:
                    t = None
                    print "Warning from Datacube.plot():',t,' is not a valid column name"
            # remove all non existing names from a list
        ...
        if y is None:
            if x is None:                             # None,None more than 2 columns => x y first and second columns
                if length >= 2:
                    x = [self.columnName(0)]
                    y = [self.columnName(1)]
                ...
                for name in names:                    # x,None => y = first non x column or row index
                    ...
                    if y is None:
                        y = ['row index']
        elif x is None:                             # None, y=> x first column if non y or row index
        ...
        for i in range(len(plots)):
            ...
            if not type in complexTypes:
                ...
                xCol = xCol.real()
                plt.plot(xCol, yCol.real(), linestyle='-', marker=marker,
                         color=colori, label=yName + '(' + xName + ')')
                plt.plot(xCol, yCol.imag(), linestyle='--',
                         marker=marker, color=colori)
                xLabel += 'Re(' + xName + ')'
    ...
    def sendToIgor(self, path="root:"):
        ...
        for column in self._meta["fieldNames"]:
            igorCom("Make /N=%i/D/O %s'%s':'%s'" %
                    (len(self[column]), path, folderName, column))
        ...
        igorCom([filenameVariable + "=\"%s\"" %
                 string.replace(self.filename(), "\\", ":")])
