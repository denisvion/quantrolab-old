...
class MyMplCanvas(FigureCanvas):
  ...
  def __init__(self, parent=None, width=5, height=5, dpi=60,name=None, threeD=False,threeDAxes=False):
    ...
    self._moveLabel.setStyleSheet("font-size:12px; margin:5px; padding:4px; background:#FFFFFF; border:2px solid #000;")
  ...
  def scroll(self,event):
    ax=event.inaxes
    x,y,xd,yd = event.x,event.y,event.xdata,event.ydata # get event location in pixel and data coordinates
    if xd is not None and yd is not None:                           # in canvas == > zoom in or out
      if self.threeDAxes:
        xd,yd,zd=[float(u[2:]) for u in event.inaxes.format_coord(xd,yd).split(', ')]
      ...
      if(self.scrollable[0]):
        ax.set_xlim([xd - new_width * (1-relx), xd + new_width * (relx)])
      if(self.scrollable[1]): 
        ax.set_ylim([yd - new_height * (1-rely), yd + new_height * (rely)])
    elif self.threeDAxes:
      inc=+5
      if event.button == 'down': inc=-5                       # outside canvas and 3D == > 3D rotate
      ax=self.axes
      elev,azim=ax.elev,ax.azim                                                  
      w,h=self.get_width_height()
      if y>h/2:                                               # scrolling  above half height of the axes == > elevation rotation
        elev+=inc                            
      else:
        azim+=inc                                             # scrolling below  == > azimutal rotation
  ...
  def onMove(self,event):
    ...
    if self._pressed:
      ...
      self.update()
    if self.threeDAxes:
      x,y,z=[float(u[2:]) for u in event.inaxes.format_coord(event.xdata,event.ydata).split(', ')]
      st="x="+str(x)+"\ny="+str(y)+"\nz="+str(z)
    else:
  ...
  def onLeave(self,e):
    if self._moveLabel.isVisible(): self._moveLabel.hide()
