"""
This module defines the HelperManager Class in charge of loading, unloading and keeping track of helpers loaded in Quantrolab.
...
class HelperManager():
  """
  The helper manager has a parent, a default helper root directory, a callable method to execute helper code,
   and memorizes information about the loaded helpers in a dictionary _helpers, the structure of which is:
  ...
  def __init__(self, parent=None, helpersRootDir=None, execute=None):
    ...
    # dictionary of the available gui or non-gui helpers. see structure above
    self._helpers = {}
  ...
  def loadHelpers(self, filename=None):
    """
    The loadHelpers method tries to load and run all HelperGUI(s) and Helper(s) defined in a single python module specified by
    ...
    If a valid .py file is found:
            ...
            6) If the associate is non-gui it is removed from the dictionary keys and appears only as an associate of the gui helper
  ...
  def runHelper(self, modulename, filename, classname, associateAttr, associateTypeName, reportName):
    ...
    code += 'execInGui(lambda : startHelper(%s,%s,%s,%s,%s,%s)' % (modulename,
                                                                   filename, classname, associateAttr, associateTypeName, reportName)
    ...
    while True:                                                                  # wait for the thread to finish
      running = self._parent._codeRunner.isExecutingCode(identifier=classname)
    ...
    while True:
      lastHelper = self._parent.processVar('lastHelper')
      if lastHelper != None or time.time() - start > timeout:
        break
    if lastHelper:
