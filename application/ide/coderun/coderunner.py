...
class CodeThread (KillableThread):
    ...
    def run(self):
        ...
        - runs resultExpression if defined and puts the result in self._result
...
class MultiProcessCodeRunner():
    ...
    def executeCode(self, *args, **kwargs):
        """
        Memorizes threadId: (callbackFunc, resultAsArg) in _callbackDict if callbackFunc is given, so that
        the _callbackQueueManager knows whether the result from resultExpression has to passed to the callback
        ...
        and returns the response from the dispatch method.
        """
        threadId = args[1]
        callbackFunc = None
