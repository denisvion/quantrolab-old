...
Note that in the Quantrolab multithread IDE, the threads in which each scripts are executed
...
def execInGui(f):
  ...
  """
  _ensureGuiThreadIsRunning()    # This test the existence of the Qt application and creates it if needed.
  global app
  if app is None:
    raise Exception("Invalid application handle!")
  app.emit(SIGNAL("runGuiCode(PyQt_PyObject)"),f) # This tells the application to run f
