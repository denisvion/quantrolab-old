...
class PrefAutoDialog(QDialog,Debugger):
    ...
    def __init__(self,parent=None,prefDict=None):
        ...
        # buid a simple representation of the preference dictionnary with Qwidgets
        if self._prefDict :
            for key in self._prefDict:
                ...
                if typ == bool: # QCheckBox
                    ...
                    widget=QComboBox()
                    widget.addItems(map(lambda x: QString(str(x)),dicti['choices']))
                    index=0
                    while index < widget.count() and widget.itemText(index) != QString(str(dicti['value'])): index+=1
                    widget.setCurrentIndex(index)
                    l.addWidget(QLabel(QString(dicti['label'])),l.rowCount(),0)
                    l.addWidget(widget,l.rowCount()-1,1)
                else : # put all other cases (int,long,double,complex,float) in QLIneEdit widgets
