...
helperDic = {'name': 'Variable Manager', 'version': 'beta 0.1', 'authors': 'V. Schmitt',
...
def startHelperGui(exitWhenClosed=False, parent=None, globals={}):
    ...
    VarManager = VarManager(parent, globals)
    # show its window
    VarManager.show()
...
class VarManager(HelperGUI):
    ...
    def __init__(self, name=None, parent=None, globals={}):
        ...
        self.connect(self._updateButton, SIGNAL(
            "clicked()"), self.updatePressed)

        self._variablesList = VariablesList(parent=self, globals=globals)
        self._props = ThreadProperties(parent=self, globals=globals)
        self.connect(self._threadList, SIGNAL(
            "currentItemChanged(QTreeWidgetItem *,QTreeWidgetItem *)"), self.selectThread)
...
class VariablesList(QWidget, ObserverWidget):
    ...
    def valueChanged(self, item):
        ...
        try:
            self._coderunner._lv[self._threadId][item] = old.__new__(
                type(old), str(self.variables[item].text()))
        except ValueError:
            messageBox = QMessageBox(
                QMessageBox.Information, "Type Error", "Value does not match previous data type")
            messageBox.exec_()
    ...
    def updateVariableList(self):
        ...
        for item in sorted(lv.keys()):
            listSkipped = ["ismodule", "isclass", "ismethod", "isfunction", "isgeneratorfunction", "isgenerator", "istraceback", "isframe",
                           "iscode", "isbuiltin", "isroutine", "isabstract", "ismethoddescriptor", "isdatadescriptor", "isgetsetdescriptor", "ismemberdescriptor"]
            ...
            if a == False:
                ...
                self.variables[item].returnPressed.connect(
                    partial(self.valueChanged, item=item))
