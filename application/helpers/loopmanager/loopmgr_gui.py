...
helperDic = {'name':'Loop Manager','version':'1.1','authors':'A. Dewes-V. Schmitt - D. Vion','mail':'denis.vion@cea.fr','start':'startHelperGui','stop':None}

def startHelperGui(exitWhenClosed = False,parent=None,globals={}):    # 3) Start the dataManager
    global loopManager                                                # define dataManager as a global variable
    loopManager = LoopManager(parent,globals)                         # Instantiate the datamanager gui here
    loopManager.show()                                                # show its window
...
class LoopManager(HelperGUI):
  ...
  def __init__(self,parent = None,globals = {}):
    ...
    """
    loopMgr = LoopMgr(parent,globals)       # instantiates the backend manager
    loopMgr._gui=self                       # inform the helper it has an associated gui by adding the gui as an attribute
    # init superClasses
    ...
    self.connect(self.autoReverseBox,SIGNAL("stateChanged(int)"), self.autoReverse)
    self._iconsBar.addWidget(self.autoReverseBox,0,7)

    self.autoLoopBox=QCheckBox('Auto-loop')
    #self.autoReverseButton.setIcon(self._icons['autoReverseButton'])
    self.connect(self.autoLoopBox,SIGNAL("stateChanged(int)"), self.autoLoop)
    self._iconsBar.addWidget(self.autoLoopBox,0,8)

    self.autoDeleteBox=QCheckBox('Auto-delete')
    #self.autoReverseButton.setIcon(self._icons['autoReverseButton'])
    self.connect(self.autoDeleteBox,SIGNAL("stateChanged(int)"), self.autoDelete)
    ...
    self.connect(self._looplist,SIGNAL("currentItemChanged(QTreeWidgetItem *,QTreeWidgetItem *)"),self.selectLoop)
    self.connect(self._looplist,SIGNAL("itemDoubleClicked(QTreeWidgetItem *,int)"),self.editVariable)
  ...
  def updatedGui(self,subject,property = None,value = None):
    ...
    """
    #print 'in updatedGui with subject = ',subject,', property = ',property,', value = ',value
    if property=='addLoop':
      self.addLoop(value)               # loop will be added at the correct level if not already present
    elif property=='addChild':
      self.addChild(value)
    elif property == "removeLoop":
      self.removeLoop(value)            # loop will be removed with all its children if present
    elif property == "removeChild":
        self.childAtTop(value)  # child will be moved at top level if present
  ...
  def updateLoop(self,loop=None):
    ...
    if loop is not None:
      fp=loop.getParams()
      autoRev,autoLoop,autoDel=fp['mode']=='autoRev',fp['mode']=='autoLoop',fp['autoDel']
    # update button bar
    ...
    # update loop item
    if loop!=None:
      item = self._items[self.ref(loop)] # look in the loop dictionary for the passed loop to retrieve the proper QTreeWidgetItem
      item.setText(0,fp['name'])         # item is the QTreeWidgetItem corresponding to the loop
      item.setText(1,loop.__class__.__name__)
      li1=start,stop,step,index,value,mode,nextValue,steps2Go=fp['start'],fp['stop'],fp['step'],fp['index'],fp['value'],fp['mode'],fp['nextValue'],fp['steps2Go']
      for i,v in zip([2,3,4,5,6,7,8,9],li1):
        if v is None :  v=''
        item.setText(i,str(v))
      timeEstim=fp['time2Go']
      if isinstance(timeEstim,(float)):
        timeEstim=time.strftime("%H:%M:%S", time.gmtime(fp['time2Go']))
      else:
        timeEstim=''
  ...
  def addLoop(self,loop):
    ...
    if self.ref(loop) in self._items: # do nothing if the loop is already present at any level
      return
    item = QTreeWidgetItem()          # prepare the item
    item.setFlags(Qt.ItemIsSelectable | Qt.ItemIsEnabled | Qt.ItemIsEditable)
    item._loop=self.ref(loop)
    loop.attach(self)
    self._items[self.ref(loop)]=item  # add to dictionary whatever future level
    parent = loop.parent()
    if parent is None or self.ref(parent) not in self._items: # if the loop has no parent already present
      #print 'inserting top level item for ',loop.getName()
      self._looplist.insertTopLevelItem(self._looplist.topLevelItemCount(),item) # insert it at the top level
    else:                                                     # else add it as a child item
      #print 'adding child item ', loop.getName(), ' to parent item ', parent.getName()
      self._items[self.ref(parent)].addChild(item)            #   (this is the Qt addChild method of QTreeWidget)
  ...
  def addChild(self,child):
    ...
    if self.ref(child) in self._items:        # if child already present (could be improved by testing also if it is not at the right place)
  ...
  def removeLoop(self,loop,update=True):
    ...
    for child in loop.children():      # call recursively removeLoop to remove the children first.
      self.removeLoop(child,update=False) # (easy way to remove them from the dictionary)
    ...
    if parent:                         # if parent exist
      ...
      self._looplist.takeTopLevelItem(self._looplist.indexOfTopLevelItem(item)) # remove it from the QTreeWidget
    del self._items[self.ref(loop)]    # and from the dictionary
    loop.detach(self)                  # detach the loop so that it stops sending notifications directly to this loopspanel
    try:
      if self._selectedLoop == loop:
        self._selectedLoop = None
    except:
      print 'an error occured in removeLoop'
    if update:  self.updateLoop()      # update only once at the end cause update is false for children
  ...
  def childAtTop(self,loop):
    ...
    if self._looplist.indexOfTopLevelItem(item)==-1:
      ...
      self._looplist.insertTopLevelItem(self._looplist.topLevelItemCount(),item)
  ...
  def editVariable(self,item,colIndex):
    ...
    if colIndex in [0,2,3,4,8,9] or (colIndex ==9 and item.text(colIndex) not in ['inf','?'] ):
      ...
      try:
        val=int(textValue)
        newVal,ok = QInputDialog().getInt(self,'Get new integer value', 'New % s =' % columnName,value=val)
      except:
        try:
          val=float(textValue)
          if 1e-10<val<1e10:
            newVal,ok = QInputDialog().getDouble(self,'Get new real value', 'New % s =' % columnName,value=val,decimals=10)
          else:
            while True:
              newVal,ok = QInputDialog().getText(self,'Get real value', 'New % s =' % columnName,text=textValue)
              try:
                newVal=float(newVal)
                break
              except:
          ...
          newVal,ok = QInputDialog().getText(self,'Get text', 'New % s =' % columnName,text=val)
          newVal=str(newVal)
      if ok and newVal!=val:
        functions=['setName',None,'setStart','setStop','setStep',None,None,None,'jumpToValue','setNSteps2Go',None]
...
class LoopList(QTreeWidget):
  ...
  def __init__(self):
    ...
    self.setHeaderLabels(["Name","Class","Start","Stop","Step","Index","Value","Mode","Next","Steps to go","Time estim.","Specific"])
