...
class DataMgr(Singleton,ThreadedDispatcher,Helper):  
  ...
  def addDatacube(self,datacube, checkTopLevelOnly=False):
    ...
    if not datacube in cubes:
      ...
      self.debugPrint('DataManager notifying "addDatacube" for datacube',datacube)
  ...
  def removeDatacube(self,datacube,deleteCube=False):
    ...
    if datacube in cubes:                             # the cube is at the top level of the hyerarchy
      ...
      self.debugPrint('DataManager notifying "removeDatacube" for datacube',datacube)
      ...
      while parent is not None and not parent in cubes :    # check that it has a parent or ascendent in the datamanager
  ...
  def plot(self,datacube,*args,**kwargs):
    ...
    self.debugPrint('DataManager notifying ""plot"" for datacube ',datacube)
