...
import sys,getopt,os,os.path,weakref,gc,time,warnings,numpy
...
# Global variables
__styles2D__=['line','scatter','line+symbol'];__defaultStyle2D__=0                                              # list of available styles for curves in a 2d graph
__styles3D__=['Image(2D)','Contours(2-3D)','Filled contours(2-3D)','Waterfall(3D)','Scatter(3D)','Line(3D)','Surface(3D)','Tri-surface(3D)'];
__defaultStyle3D__=0      # list of available styles for curves in a 3d graph
__styles3DAxes3DOnly__=['Waterfall(3D)','Scatter(3D)','Surface(3D)','Tri-surface(3D)','Line(3D)']                                          # list of 3D styles that can be shown only on the x,y,z 3D frame of a matplotlib Axes3D
__styles3DAxes2DOnly__=['Image(2D)']                                                                          # list of 3D styles that can be shown only in the xy plane of a matplotlib Axes
__styles3DEncode111__=['Scatter(3D)','Line(3D)','Tri-surface(3D)']                                                                 # list of 3D styles that will be encoded as 1D x, y, and z lists
__styles3DEncodexx2__=['Image(2D)','Contours(2-3D)','Filled contours(2-3D)','Waterfall(3D)']                                                   # list of 3D styles that will be encoded with x, y and z dimensions 2,1,2 or 1,2,2
__styles3DEncode222__=['Surface(3D)']
__styles3DRectangularOnly__=['Image(2D)','Contours(2-3D)','Filled contours(2-3D)','Surface(3D)']                                      # list of 3D styles that can be shown only on a rectangular x,y grid
__styles3DRegularOnly__=['Image(2D)']                                                                         # list of 3D styles that can be shown only on a regular x,y grid (i.,e. equally spaced along x and y)
__colors3D__=['binary','gray','jet','hsv','autumn','cool'];__defaultColor3D__=2                                 # list of matplotlib color schemes
__regridize__=['None','skip','interpolate'];__defaultRegridize__=0                                              # list of methods to regularize unregular data (not programmed yet)
...
class PlotWidget(QWidget,ObserverWidget,Debugger):
    ...
    def __init__(self, parent=None, dataManager=None, name=None, threeD=False):  # creator PlotWidget
        ...
        self._dataManager = dataManager           # attach  dataManager to this Plot2DWidget, i.e. Plot2DWidget receives message from datamanager  
    ...
    def removeFromCanvas(self,plot):
        ...
        if not self._threeD:
            ...
            try:                  
                ...
                self.updatePlot(clearFirst=True,draw=True)  # graphical object has no remove attribute and canvas has to be reconstructed from scratch
    ...
    def updatePlot(self,listOfPlots=None,draw=False,clearFirst=False):
        ...
        """
        self.debugPrint('in PlotWidget ',self.name,'.updatePlot() with draw = ',draw)
        if clearFirst:  self.canvas.axes.cla()
        ...
        if len(self._plots) == 0:                            # clear the graph without using clf, which would loose the fixed scales if any.
            self.canvas.redraw()                                   
            return
        if listOfPlots is None:   listOfPlots=self._plots
        if listOfPlots is None or listOfPlots == [] : return
    ...
    def setCube(self,cube,names=[None,None,None]):
        ...
        """
        self.debugPrint('in PlotWidget.setCube(cube,names) with cube =',cube,' names=',names)  
        detachPrevious= True
    ...
    def removePlot(self,plot,update=True):
        ...
        """
        self.debugPrint("in PlotWidget.removePlot() with plot = ",plot," and update = ",update)
        if plot:
...
class Plot2DWidget(PlotWidget):
    ...
    def __init__(self, parent=None, dataManager=None, name=None): # creator Plot2DWidget
        PlotWidget.__init__(self,parent=parent,dataManager=dataManager,name=name,threeD=False)
    ...
    def updatedGui(self,subject = None,property = None,value = None):     # notification listener of Plot2DWidget
        self.debugPrint('in ',self.name,'.updatedGui with subject=', subject,', property=',property,', and value=',value)
        if subject == self._dataManager and property == "addDatacube" :           # 1) A new datacube arrived.
        ...
        elif subject == self._dataManager and property == "addChild" :            #  4) a datacube of the datamanger not attached to this plotter (otherwise would be case 3) has new child
            if self.autoplot.isChecked():
    ...
    def addPlot(self,cube=None,level=None,names=[None,None],clear='auto',style=None,**kwargs):
        ...
        """
        self.debugPrint('in ',self.name,'.addPlot with cube=',cube,',level=',level,', names=',names,', clear=',clear,', style=',style,', and current cube = ',self._cube)          
        if level is None: level=self.level.value()
        if cube is None : cube =self._cube              # if no datacube passed use the current datacube
        if cube is None:  return                        # give-up if no datacube
        if len(cube.names()) == 0:                      # if datacube has no column,...
            if self.autoplot.isChecked():
                self.debugPrint('attaching ',cube,' to ',self)  # attach it  if autoplot... 
    ...
    def addPlot2(self,cube=None,names=[None,None],clear='auto',style=None,**kwargs):
        ...
        """
        self.debugPrint('in ',self.name,'.addPlot2 with cube=',cube,', names=',names,', clear=',clear,', style= ', style, ' and current cube = ', self._cube)
        if cube is None : cube =self._cube              # if no datacube passed use the current datacube
        if cube is None:  return                        # give-up if no datacube
    ...
    def updatePlotData(self,plot=None,draw=False,**kwargs):
        ...
        If no list of plots is given, all plots in self._plots are updated.
        """
        self.debugPrint('in ',self.name,'.updatePlotData() with plot=',plot,' and draw =',draw)
        if plot is None and self._currentIndex:   plot=self._plots[self._currentIndex]
        if plot:
    ...
    def updateControls(self,level=None,names=[None,None]):
        ...
        """
        self.debugPrint("in Plot2DWidget.updateControls(level,names) with level=",level,", names=",names,', and current datacube=',self._cube)
        cube=self._cube
        names=names[0:2]
        selectors=[self.xNames,self.yNames]
        ...
        if cube is not None:
          ...
          if level is not None : self.level.setValue(level)
    ...
    def preselectVariables(self,cube=None,level=0,names=[None,None],previousNames=[None,None]):
        ...
        """
        self.debugPrint('in Plot2DWidget.preselectVariables(cube,level,names,previousNames) with cube = ',cube,' level = ',level,', names = ',names,', and previousNames = ',previousNames)
        if cube is None : cube=self._cube           # if no datacube passed use the current datacube
        if cube is None :  return [None,None]       # if no datacube give up
        if level is None :level=0                   # prepare a plot at level 0 if no level speciifed
        commonNames=cube.commonNames()            # gets all column names of the datacube and its children that are common to a same level
    ...
    def setStyle(self,index=None,style=None):
        self.debugPrint("in Plot2DWidget.setStyle() with index=",index,"and style=",style)
        if index is None :                                                # if index is not passed choose the current index in plotList
    ...
    def setColor(self,index=None,color=None):                            
        self.debugPrint("in Plot2DWidget.setColor() with index=",index,"and color=",color)
        if index is None :                                                 # if index is not passed choose the current index in plotList
          index=self.plotList.indexOfTopLevelItem(self.plotList.currentItem())
...
class Plot3DWidget(PlotWidget):
    ...
    def __init__(self, parent=None, name=None, dataManager=None):         # creator Plot3DWidget
        PlotWidget.__init__(self,parent=parent,dataManager=dataManager,name=name,threeD=True)
    ...
    def updatedGui(self,subject = None,property = None,value = None):     # notification listener of Plot3DWidget
        ...
        """
        self.debugPrint('in ',self.name,'.updatedGui with subject=', subject,', property=',property,', and value=',value)
        if subject == self._dataManager and property == "addDatacube" :           # 1) A new datacube arrived in the datamanager.
            ...
            if  self._cube is not None and (cube == self._cube or cube in self._cube.children()):       #     if cube = current datacube or is child of current datacube
            ...
            if self.autoplot.isChecked() :                                #     if autoplot
                ...
                test=parent is not None and self in parent.observers() and parent not in [plot.cube for plot in self._plots]  #         if yes                                                                                 
                if test:
            ...
            if plot:
                ...
                if needChild:                                             #     
                    self.debugPrint('attaching ',child,' to ',self)               #     => attach child to plot3DWidget
                    child.attach(self)
    ...
    def switchAxes2D3D (self,newStyle,newPlot=None):
        ...
        self.debugPrint('new style=',newStyle,'threeDAxes=',self.canvas.threeDAxes,'toAxes3D=',toAxes3D,'toAxes=',toAxes)
        switched=False
        if toAxes3D or toAxes:
            switch=len(self._plots) == 0 or (len(self._plots) == 1 and self._plots[0] == newPlot) 
            switch =switch or QMessageBox.question (self,'Warning', 'Current plot(s) will be cleared. Confirm?',QMessageBox.Ok, QMessageBox.Cancel) == QMessageBox.Ok
            if switch :    
                self.clearPlots()
    ...
    def addPlot(self,cube=None,names=[None,None,None],clear='auto',style=None,color=None,**kwargs): # subclass in Plot2DWidget and Plot3DWidget
        ...
        """
        self.debugPrint('in ',self.name,'.addPlot with cube=',cube,', names=',names,', clear=',clear,', style= ', style, ' and current cube = ', self._cube)
        if cube is None : cube =self._cube                            # if no datacube passed use the current datacube
        if cube is None:  return                                      # give-up if no datacube
        if (self.autoclear.isChecked() and clear == 'auto') or clear == True:       # clear if autoclear
          self.clearPlots() 
        ...
        self.debugPrint('attaching ',plot.cube,' to ',self)         # attach cube to plot3DWidget...
        plot.cube.attach(self)
        needChildren=any([('child:' in name) for name in names ])
        if needChildren:
            for child in cube.children():                                      
                self.debugPrint('attaching ',child,' to ',self)     # ... and its relevant children for the plot 
                child.attach(self)
        ...
        plot.MPLplot=None                                           # initialize empty plot3D
        self._plots.append(plot)                                    # and add the plot to the list self._plots
        self._updated = False                                       # let ontimer() call all functions updating the graph with points, create all labels, and redraw if necessary)
    ...
    def updatePlotData(self,draw=False,plot=None,regularize=None,**kwargs):
        ...
        self.data2Plot(plot,regularize=regularize)                         # call the builder of data dictionary "plot.data".
        dat=plot.data
        style=plot.style
        x,y,z,dims=dat['x'],dat['y'],dat['z'],dat['xyzDims']
        pl=plot.MPLplot
        if style == "Waterfall(3D)":         # plot.MPLplot is a list of couples(ui, linei)
            u1d,u2d=x,y
            if dims[0] == 2:  u1d,u2d=y,x
            if pl is None:    pl=[]
            ...
            for i in range(len(u1d)):
                if u1d[i] in uis:
                    ...
                    oldLine=pl[i][1]
                    newLine=Line3D(array([u1d[i]]*len(u2d[i])),u2d[i],z[i])
            ...
            if dat['regular']:
                ...
                if dims[0] == 1:                                               
                    ...
                    dat['mask']=dat['mask'].T
                if pl is None:
                    if len(dat['z']) != 0:                                    # this is where the image or countour plot.MPLplot is created 
                        if style == "Image(2D)":
                            pl=plot.MPLplot = ax.imshow(ma.array(dat['z'],mask=dat['mask']),interpolation='nearest',cmap=cmap,origin='lower',**kwargs) # create 2D image
                            self.setExtent2DImage(pl,dat['xMin'],dat['xMax'],dat['yMin'],dat['yMax'],dat['dx'],dat['dy'])
                        elif style in ["Contours(2-3D)","Filled contours(2-3D)"]:
                            if dims[0] == 2:  x=x[0]
                            if dims[1] == 2:  y=y[0]
                            plotMethod=ax.contour
                            if style == "Filled contours(2-3D)":   plotMethod=ax.contourf
                            pl=plot.MPLplot = plotMethod(x,y,ma.array(dat['z'],mask=dat['mask']),cmap=cmap)
                        self.addColorBar(pl,label=plot.zname)                   # and its colorbar        
                    ...
                    if style == "Image(2D)":
                        ...
                        if dims[1] == 2:  y=y[0]
                        plotMethod=ax.contour
                        if plot.style == "Filled contours(2-3D)":   plotMethod=ax.contourf
                        pl=plot.MPLplot = plotMethod(x,y,ma.array(dat['z'],mask=dat['mask']),cmap=cmap) # no possible update. Recalculate
            ...
            if len (dat['z']) != 0:
                pl=plot.MPLplot = ax.plot_trisurf(x,y,z,cmap=cmap,linewidth=0.2)
    ...
    def collectData(self,plot):
        ...
        dims=[]                                                                             # dim stores the corresponding dimensions [dim[x],dim[y],dim[z]]                                                                       
        #xLinesInz=None
        for name in names:
            if name.startswith('childAttr:'):                                               # If the variable is a child attribute
                ...
                if name == '[row]':                                                           # If the variable is one of the column of the cube
    ...
    def to1d(self,dims,rowxyz,xLinesInz=True):
        ...
        if dims == [1,1,2]:                         # dimensions are 1d,1d,2d
            lx,ly,lz =[len(u) for u in rowxyz]   # calculate lengthes of x, y, and z
            lz2=[len(u) for u in rowxyz[2]]      # calculate lengthes of z[1], z[2], ...
            if xLinesInz == True:                     # z= [xListAty1, xListAty2, xListAty3,...]
                ly=min(ly,lz)
                for i in range(ly):
                    lxi=min(lx,lz2[i])
            ...
            elif xLinesInz == False:                  # z= [yListAtx1, yListAtx2, yListAtx3,...]
                lx=min(lx,lz)
                for i in range(lx):
                    lyi=min(ly,lz2[i])
            ...
            lx,ly,lz =[len(u) for u in rowxyz]   # calculate lengthes of x, y, and z
            lx=min(lx,ly,lz)
            ly2=[len(u) for u in rowxyz[1]]      # calculate lengthes of z[1], z[2], ...
            lz2=[len(u) for u in rowxyz[2]]      # calculate lengthes of z[1], z[2], ...
            for i in range(lx):
                lyi=min(ly2[i],lz2[i])
            ...
            lx,ly,lz =[len(u) for u in rowxyz]   # calculate lengthes of x, y, and z
            ly=min(lx,ly,lz)
            lx2=[len(u) for u in rowxyz[0]]      # calculate lengthes of x[1], x[2], ...
            lz2=[len(u) for u in rowxyz[2]]      # calculate lengthes of z[1], z[2], ...
            for i in range(ly):
                lxi=min(lx2[i],lz2[i])
    ...
    def to2d(self,dims,rowxyz,sort=None):
        ...
        if sort == 'x':
            ...
            pass # to be coded
        i1d,i2d=0,1                                                                     #   y will be the 2d variable except if x[0] and x[1] exist and are different
        if len(rowxyz[0])>1 and rowxyz[0][1] != rowxyz[0][0]: i1d,i2d=1,0                                                                                 
        # reshape the 1d and 2d variables:
        rowxyz[i1d],pos=array([(u,n) for (n,u) in enumerate(rowxyz[i1d]) if n == 0 or u != rowxyz[i1d][n-1]]).transpose() # get the list of values and position of var1d elements different from the previous one
        rowxyz[i2d]=[rowxyz[i2d][pos[i]:pos[i+1]] for i in range(len(pos)-1)]+ [rowxyz[i2d][pos[len(pos)-1]:]]        # use position to slice the 2d variable
        rowxyz[2]=array([rowxyz[2][pos[i]:pos[i+1]] for i in range(len(pos)-1)]+ [rowxyz[2][pos[len(pos)-1]:]])       # and the z. make an array
        dims[i2d]=2
        dims[2]=2
    ...
    def to222d(self,dims,rowxyz,sort=None): # TO BE CODED
        ...
        if dims[2] == 1: self.to2d(dims,rowxyz,sort=sort)
        if dims[0:1] == [1,2]:
    ...
    def data2Plot(self,plot,regularize=None):
        ...
        """
        self.debugPrint('in ',self.name,'.data2Plot() with plot=',plot," and regularize =",regularize )
        dims,rowxyz=self.collectData(plot)
        if dims == [1,1,1] and plot.style in __styles3DEncodexx2__:                    # call translator to2d if needed 
            self.to2d(dims,rowxyz,sort=None)
        elif dims != [1,1,1] and plot.style in __styles3DEncode111__:                  # call translator to1d if needed
            self.to1d(dims,rowxyz)
        elif dims != [2,2,2] and plot.style in __styles3DEncode222__:                  # call translator to222d if needed
            self.to222d(dims,rowxyz) 
        plot.data={}
        plot.data['xyzDims']= dims                                                   
        plot.data['x'], plot.data['y'],plot.data['z']=rowxyz
        if plot.style in __styles3DRectangularOnly__ + __styles3DRegularOnly__:       # In case the type of matplotlib plot requires regular data
        # The goal is now to build a valid rectangular and regular 2d array dat['z'] and its mask dat[mask'] for building later a masked array
            ...
            if (not plot.data['rectangular'] or not plot.data['regular']) and regularize:               # if not regular and regularize requested
            ...
            if XYMatchZ:
                ...
                if rectangular:
                    ...
                    if len(var2d[0]) == 0:    regular2D=False
                        ...
                        regular2D=all([abs(dyi/dys[0]-1.)<1.e-4 for dyi in dys])
                        self.debugPrint('regular2D = ',regular2D,', MindX2D = ',min(dys),', MaxdX2D = ',max(dys))
                        if regular2D:   dy=dys[0]
                    if len(var1d) == 0:       regular1D=False
                    elif len(var1d) == 1:     regular1D=True
                    else:
                        dxs= map (lambda a,b: a-b,var1d[1:],var1d[:-1])
                        regular1D=all([abs(dxi/dxs[0]-1.)<1.e-4 for dxi in dxs])
                        self.debugPrint('regular1D = ',regular1D,', MindX1D = ',min(dxs),', MaxdX1D = ',max(dxs))
                        if regular1D:   dx=dxs[0]
                    regular=regular2D and regular1D
                    if dims[0] == 2:  dx,dy=dy,dx
                    if not full:                          # although missing=true will be saved in plot.data['missing'],  
    ...
    def regularize(self,plot):
        ...
        dat=plot.data
        rectangular,regular,xDim,yDim=dat['rectangular'],dat['regular'],data['xyzDims'][0:2]
        if rectangular and not regular:
            xs0 = dat['x']
            if xDim == 2: xs0=xs0[0]
            xMin,xMax=min(xs0),max(xs0)
            dx=min(set(map(lambda a,b: a-b,xs0[1:],xs0[:-1])).discard(0)) # smallest dx different from 0
            ys0 =dat['y']
            if yDim == 2: ys0=ys0[0]
            yMin,yMax=min(ys0),max(ys0)
            dy=min(set(map(lambda a,b: a-b,ys0[1:],ys0[:-1])).discard(0)) # smallest dy different from 0
            zs0 =dat['z']
            xs,ys = np.mgrid[xMin:xMax:int((xMax-xMin)/dx)+1, yMin:yMax:int((yMax-yMin)/dy)+1]
    ...
    def updateControls(self,names=[None,None,None]):                    # subclass in Plot2DWidget and Plot3DWidget
        ...
        """
        self.debugPrint('in Plot3DWidget.updateControls(names) with names=',names,', and current datacube=',self._cube)
        ...
        if cube is not None:                                     # if current cube exists
            ...
            for selector in selectors :
                # check if any name in names0, names1, or attributes is absent from existing selector
                if any([selector.findText(name) == -1 for name in names0+names1+attributes]):   # if missing element 
                    self.regularGrid.setChecked(False)                                        # refill the selector
                    selector.clear()           
            ...
            if  names and names != [None,None,None] and names != currentNames and all([bool(name) for name in names]):     # select the requested x and y names if correct and necessary
                indices=map(lambda selector,name:selector.findText(name),selectors,names)
                if all(index != -1 for index in indices):
                    map(lambda selector,index:selector.setCurrentIndex(index),selectors,indices)
    ...
    def preselectVariables(self,cube=None,names=[None,None,None],previousNames=[None,None,None]):   # subclass in Plot2DWidget and Plot3DWidget
        ...
        if all([(name and name in allNames) for name in names]): # if all passed names exist select them
            ...
            parentColumn=cube.column(cube.columnName(0)) # if at least one column after row in parent, with a length ...
            if len(names0)>=2 and parentColumn is not None and len(parentColumn) == len(children): # ... equal to the number of children
    ...
    def setStyle(self,index=None,style=None):                           # subclass in Plot2DWidget and Plot3DWidget
        self.debugPrint("in Plot3DWidget.setStyle() with index=",index,"and style=",style)
        if index is None :                                                # if index is not passed choose the current index in plotList
          index=self.plotList.indexOfTopLevelItem(self.plotList.currentItem())
        if index == -1:                                
          return                                                        # return if not valid index
        ...
        if not style:                                                   # if style is not passed choose the current default style
          style = self.styles.itemData(index).toString()
        if self.style == style:                                         # if style is already correct do nothing and return
          return
        plot.style=style
        switched=self.switchAxes2D3D(style,plot)                        # switch between 2D axes and 3D axes if needed and accepted by user
        if not switched:
            plot.MPLplot=None                                           
            self._plots[self._currentIndex].style = style
            ...
            cube,names,style,color=plot.cube,[plot.xname,plot.yname,plot.zname],plot.style,plot.color
            del plot
    ...
    def setColor(self,index=None,color=None):                           # subclass in Plot3DWidget
        self.debugPrint("in Plot3DWidget.setColor() with index=",index,"and color=",color)
        if index is None :                                                # if index is not passed choose the current index in plotList
          index=self.plotList.indexOfTopLevelItem(self.plotList.currentItem())
        if self._currentIndex == -1:                                
          return                                                        # return if not valid index
    ...
    def setExtent2DImage(self,pl,xMin,xMax,yMin,yMax,dx,dy):
        ...
        def lowHigh(uMin,uMax,du):                           # set the x and y limits of the image
            uLow=uMin-abs(du)/2.
            uHigh=uMax+abs(du)/2.
            if du<0: uLow,uHigh=uHigh,uLow
            return uLow,uHigh
    ...
    def addColorBar(self,MPLplot,label='z'):
        ...
        if len(axesList) == 1:
            ax0=axesList[0]
            if not hasattr(ax0, 'initialPosition'):                     # memorize data axes position for future restore
            ...
            if hasattr(ax0, 'colorbarPosition'):
                ...
                ax0.colorbarPosition=cb.ax.get_position()               # memorize colorbar position for future use
