...
class CubeProperties(QWidget, ObserverWidget, Debugger):
    ...
    def __init__(self, parent=None, globals={}):
        ...
        # use an event corresponding to manual editing editing
        self.connect(self._parameters, SIGNAL(
            "cellChanged(int,int)"), self.parameterToDict)

        self.bindName = QLineEdit()
        self.updateBindButton = QPushButton("Update / Set")
        self.updateBindButton.setSizePolicy(
            QSizePolicy.Maximum, QSizePolicy.Maximum)
        self.connect(self.updateBindButton, SIGNAL(
            "clicked()"), self.updateBind)
        ...
        self.connect(self._name, SIGNAL(
            "textEdited(QString)"), self.nameChanged)
        self.connect(self._tags, SIGNAL(
            "textEdited(QString)"), self.tagsChanged)
        self.connect(self._description, SIGNAL(
            "textChanged()"), self.descriptionChanged)
    ...
    def parameterToDict(self, i, j):
        # Types of existing parameters different from numbers are preserved and
        # cannot change
        key = self._parameters.item(i, 0)
        if key:
            # New parameters are made int if they can be interpreted as int,
            # and float if they can be interpreted as float;
            key = str(key.text())
        # but other types are not recognized and are stored as strings
        newValue = str(self._parameters.item(i, 1).text())
        try:
            # first try to interpret the attribute as python code
    ...
    def updateProperties(self):
        ...
        if self._cube is not None:
            ...
            for key in params:                                        # fill parameters
                ...
                self._parameters.setItem(
                    i, 1, QTableWidgetItem(str(params[key])))
                i += 1
            ...
            if parent is not None:                                          # fill child attributes if cube has a parent
                ...
                if 'row' in attribs:
                    ...
                    self._attributes.setItem(
                        i, 1, QTableWidgetItem(str(attribs['row'])))
                    del attribs['row']
                    i += 1
                for key in attribs:
                    self._attributes.setItem(i, 0, QTableWidgetItem(str(key)))
                    self._attributes.setItem(
                        i, 1, QTableWidgetItem(str(attribs[key])))
                    i += 1
            else:
    ...
    def updatedGui(self, subject=None, property=None, value=None):
        self.debugPrint(
            "in DatacubeProperties.updatedGui with property ", property, ' and value=', value)
