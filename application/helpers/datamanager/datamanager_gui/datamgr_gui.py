...
import sys,getopt,os,os.path,weakref,gc,time,warnings
...
from application.helpers.datamanager.datamanager_gui.datacubeview import *              # DatacubeView
from application.helpers.datamanager.datamanager_gui.cubeproperties import *            # CubeProperties
...
class DataManager(HelperGUI):
    ...
    def selectCube(self,current,last):
        ...
        if current is None: self._cube=None
        else:
          self._cube = current._cube()
          if self._cube is None: self.cubeList.removeItem(current)
        cube=self._cube
    ...
    def loadCube(self):
        ...
        filename = QFileDialog.getOpenFileName(filter = "Datacubes (*.par);;Datacubes (*.txt)",directory = self.workingDirectory())
        if not filename == '':
          ...
          self._cube=cube                     # Make the manually loaded datacube the current cube. It will be automatically selected also in the CubeTreeView
    ...
    def newCube(self):
        ...
        self._helper.addDatacube(cube)
        self._cube=cube                     # Make the manually created datacube the current cube. It will be automatically selected also in the CubeTreeView
    ...
    def markAsBad(self):
        if userAsk("Are you sure ?",timeOut=5,defaultValue=False):
          ...
          if "badData" in self._cube.parameters() and self._cube.parameters()["badData"] == True:
            ...
            return
          self.removeCube()
          self._cube.savetxt(os.path.normpath(subDir+"/"+self._cube.name()),folders=self.pref('folders'),header=self.pref('header'))
    ...
    def saveCube(self,saveAs = False):
        ...
        if self._cube.filename() is None or self._cube.filename() == '' or saveAs:
          ...
          filename = QFileDialog.getSaveFileName(filter = filter1,directory = directory)
          if filename != "":
            self.setWorkingDirectory(filename)
            self._cube.savetxt(str(filename),overwrite=True,folders=self.pref('folders'),header=self.pref('header'))
        else:
          header=self._cube.filename().split('.')[-1] == 'txt' # save as txt if name ends with .txt and as par otherwise.
          self._cube.savetxt(overwrite=True,folders=self.pref('folders'),header=header )
    ...
    def add2DPlotter(self):
        plotter2D = Plot2DWidget(parent=self,dataManager=self._helper,name="2D Data plot "+str(len(self.plotters2D)+1))
    ...
    def add3DPlotter(self):
        plotter3D = Plot3DWidget(parent=self,dataManager=self._helper,name="3D Data plot "+str(len(self.plotters3D)+1))
...
class CubeTreeView(QTreeWidget,ObserverWidget,Debugger):
    ...
    def __init__(self,parent = None, manager=None):
        ...
        self._parent=parent
        self._items = dict()
        self._manager = manager           # attach  dataManager to this CubeTreeView, i.e. CubeTreeView receives message from datamanager
    ...
    def renameCube(self):
        if self._parent is not None: self._parent.renameCube()
    ...
    def contextMenuEvent(self, event):
        ...
        if action == addRenameAction:
          ...
          if self._parent is not None: self._parent.addChild(new=False)
        if action == addNewChildAction:
          if self._parent is not None: self._parent.addChild(new=True)
        elif action == removeAction:
          if self._parent is not None: self._parent.removeCube()
        elif action == saveAsAction :
          if self._parent is not None: self._parent.saveCubeAs()
        elif action == markAsGoodAction:
          if self._parent is not None: self._parent.markAsGood()
        elif  action == markAsBadAction:
          if self._parent is not None: self._parent.markAsBad()
    ...
    def addCube(self,cube,parent):
        ...
        """
        self.debugPrint("in CubeTreeView.addCube(cube) with cube =",cube, "and cube's parent =",parent)
        if self.ref(cube) in self._items:     # Returns without adding if cube reference already present
          return
        ...
        if parent is None:
          ...
          self._items[self.ref(parent)].addChild(item) # this is the addChild method of QTreeWidget
    ...
    def removeCube(self,cube,parent):
        ...
        """
        self.debugPrint('in CubeTreeView.removecube(datacube) with datacube = ',cube,' and parent = ',parent)
        for child in cube.children():
    ...
    def updatedGui(self,subject,property = None,value = None):
        self.debugPrint("in CubeTreeView.updatedGui with subject=",subject," property=", property,' and value =',value)
