...
class DatacubeViewModel(QAbstractItemModel, ObserverWidget):
    ...
    def getElement(self, index):
        ...
        cube = self._cube
        table = cube._table
        if table is None or (ix >= len(table)) or (ix > len(cube)):
            return ""  # get elements up to first row out of datacube, i.e. up to index = length
        else:
            # _table is the whole table including the first element outside
            # cube whereas table() is
            value = self._cube._table[ix, iy]
    ...
    # Returns the rowCount of a given node. If the corresponding directory has
    # never been parsed, we call buildDirectory to do so.
...
class DatacubeTableView(QTableView):
    ...
    def __init__(self, cube=None, parent=None):
        ...
        self.connect(self.horizontalHeader(), SIGNAL(
            "sectionDoubleClicked(int)"), self.renameColumn)
        self.connect(self.selectionModel(), SIGNAL(
            "currentChanged(QModelIndex,QModelIndex)"), self.newSelectedItem)
    ...
    def insertRows(self, rows):
        self.dirModel.datacube().insertRows(rowIndex=min(
            rows), numberOfRows=len(rows), notify=True)
    ...
    def copyAsPlainText(self, rows, cols):
        ...
        for row in rows:
            for col in cols:
                # DV not shure when setting parent to None
    ...
    def getContextMenu(self, p):
        ...
        if len(selectedItems) >= 1:
            ...
            self.connect(copy, SIGNAL("triggered()"),
                         lambda: self.copyAsPlainText(rows, cols))
            self.connect(addRow, SIGNAL("triggered()"), self.addRow)
            self.connect(addColumn, SIGNAL("triggered()"), self.addColumn)
            self.connect(removeRows, SIGNAL("triggered()"),
                         lambda: self.removeRows(rows))
            self.connect(removeColumns, SIGNAL("triggered()"),
                         lambda: self.removeColumns(cols))
            self.connect(insertRows, SIGNAL("triggered()"),
                         lambda: self.insertRows(rows))
            self.connect(insertColumns, SIGNAL("triggered()"),
                         lambda: self.insertColumns(cols))
