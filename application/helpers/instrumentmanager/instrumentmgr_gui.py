import sys,os,os.path,imp,time,shelve,inspect
import string,yaml,pickle,traceback
...
from application.ide.editor.codeeditor import CodeEditor #(for displaying code of instruments)
...
helperDic = {'name':'Instrument Manager','version':'0.2','authors':'A. Dewes-V. Schmitt - D. Vion','mail':'denis.vion@cea.fr','start':'startHelperGui','stop':None}

iconPath = os.path.dirname(__file__)+'/resources/icons'

def startHelperGui(exitWhenClosed = False,parent=None,globals={}):      # 3) Start the dataManger
...
def startHelperGuiInGui(exitWhenClosed = False,parent=None,globals={}):           # 2) start the datamanager in IDE
...
class InstrumentManager(HelperGUI):
  ...
  def __init__(self,parent = None,globals={}):
    ...
    # init superClasses
    HelperGUI.__init__(self,parent,globals,helper=instrumentMgr) # defines it as the associated helper of the present HelperGUI
    ...
    self.prefDict={}                                    # preference dictionnary for automatic build of preferences dialog box
    ...
    widgetLeft=QWidget()
    layoutLeft = QGridLayout()                          # left layout contains the instrument list (QTreeWidget) and a few buttons
    ...
    # create dictionary of frontpanels because the backend instrument manager does not manage them
  ...
  def setWorkingDirectory(self,filename):
    ...
    """
    self.debugPrint("in InstrumentManagerPanel.setWorkingDirectory(filename) with filename=",filename)
    if filename != None:
  ...
  def openList(self):
    ...
    # choose the file
    filename = str (QFileDialog.getOpenFileName(caption='Open instrument list file',filter = "Instrument list file (*.py)",directory = self.workingDirectory()))
    if filename != '':
      # import the file that should define an attribute instruments, and call the instrument manager initInstruments method
  ...
  def updatedGui(self,subject = None,property = None,value = None):
    ...
    """
    self.debugPrint("in InstrumentManagerPanel.updatedGui() with subject=",subject," property=",property," and value=",value)
  ...
  def updateInstrumentsList(self):
    ...
    for newName in newNames:
      if newName not in oldNames:
        ...
        item.setText(0,newName) # we put only the intrument name in the QTreeViewItem
        # the instrument handle will be retrieved using self._helper.instrumentHandles()[QTreeViewItem.text(0)]
  ...
  def dockFrontpanel(self,name,show=True):
    ...
    if index != -1:                                           # instrument panel is already in a tab
      if show:
        self._tabs.setCurrentIndex(index)                   # => make active this tab if show is True
    elif isinstance(frontpanel.parent(),DockToTabWidget):   # panel is in a floating DockToTabWidget
      frontpanel.parent().show()
      frontpanel.parent().setFloating(True)
      frontpanel.parent().dockToTab()                       # => dock the widget of DockToTabWidget back to DockingTabWidget.
    else:                                                   # panel is NOT in a floating DockToTabWidget
      panelName=name+' panel'                               # => add it in a new tab of to self._tab
  ...
  def loadStates(self,path = None):
    ...
    """
    self.debugPrint("in InstrumentManagerPanel.loadStates(path) with path=",path)
    if path is None:
      path = self._picklePath
  ...
  def selectInstr(self,new,previous):
    ...
    """
    self.debugPrint('in InstrumentManagerPanel.selectInstr() with current instr =',new,' and last instr=',previous)
...
class InstrHelpWidget(QWidget):
  ...
  def __init__(self,parent=None):
    ...
    self.connect(self.methods,SIGNAL("currentItemChanged(QTreeWidgetItem *,QTreeWidgetItem *)"),self.help)
    self.connect(self.search,SIGNAL("textEdited(QString)"),self.searchF)
    self.connect(self.methodCall,SIGNAL("editingFinished()"),self.executeCommand)
    ...
    layout1.addWidget(self.generalHelp,layout1.rowCount(),0,1,layout1.columnCount())
  ...
  def fillClassSourceDoc(self):
    ...
    if doc is None : doc='No documentation string.'
    self.generalHelp.setText(QString(doc))

  def fillMethods(self):
    ins=self._instr.instrument()
    clas=ins.getClass()
    publicMethods=[method[0] for method in inspect.getmembers(clas, predicate=inspect.ismethod) if method[0][0] != '_']
  ...
  def help(self,new,previous):
    ...
    if argspec is None:
      ...
      methodCall=methodName+'('                     # start to built the method call
      nArgs=0                                       # determine the number of arguments and keyword arguments
      if args is not None:
        nArgs=len(args)
        nKwargs=0
        ...
        if nArgs != 0:                                # add arguments but self
          if args[0] != 'self':
             ...
             if nKwargs != 0: methodCall+=','
          elif nArgs>1:
            methodCall+=','.join(args[1:nArgs])
            if nKwargs != 0 or varargs is not None or keywords is not None: methodCall+=','
        if nKwargs != 0:                              # add keyword arguments
          methodCall+=','.join([kwarg+'='+str(default) for kwarg,default in zip(args[nArgs:],defaults)])
          if varargs is not None or keywords is not None: methodCall+=','
        if varargs is not None:
          methodCall+='*'+varargs
          if keywords is not None: methodCall+=','
        if keywords is not None: methodCall+='**'+keywords
        methodCall+=')'
  ...
  def searchF(self,qstr):
    if qstr.trimmed().isEmpty():
      ...
      if len(found) != 0:
        ...
        for item in found:
          self.methods.takeTopLevelItem(self.methods.indexOfTopLevelItem(item))
...
class InstrSCPIWidget(QWidget):
  ...
  def myWrite(self):
    ...
    self.log.setTextColor(QColor('black'))
    self.log.append(commandQString)
    #self.messageOut.clear() # user might want send twice the same commmand
...
class InstrumentList(QTreeWidget):
  ...
  def mouseDoubleClickEvent(self,e):
    ...
    self.emit(SIGNAL("loadShowFrontpanels()"))     # emit signal showFrontPanel
