...
II.6) Run a piece of code by selecting lines in a script window, and typing Shift+Enter or clicking the "Run selection" icon in the icon bar. (The selection is completed automatically to include the beginning of the first line and the end of the last line).
II.7) Kill a running script by clicking in its window and then clicking the "Kill thread" icon in the icon bar.

III) Visualizing current threads and their status
Each thread run in Quantrolab appears in the "Threads" tab on the left.
Each thread has an identifier that is either a name for a helper (plugin), or a number for a script.
...
A script with status "running" can be killed by selecting it in the "Threads" tab and clicking the "Kill" button below.
...
V.3) Quantrolab remembers open helpers when quitting and reopens them the next time it is restarted.
...
V.6) The "InbstrumentManager" helper is one of the standard helper in Quantrolab. It helps you controlling the instruments in your laboratory, through python drivers. See the corresponding help.
