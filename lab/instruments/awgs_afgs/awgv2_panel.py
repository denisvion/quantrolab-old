...
class ChannelWidget(QWidget, ObserverWidget):
    ...
    def updateValues(self):
        for property in self._properties:
            getattr(self, property).setText(
                str(getattr(self._awg, property)(self._channel)))

    def setValues(self):
        for property in self._properties:
            getattr(self._awg, "set" + property[0].capitalize() + property[1:])(
                self._channel, float(getattr(self, property).text()))
        self.updateValues()
    ...
    def __init__(self, awg, channel, parent=None):
        ...
        self._properties = ["amplitude", "offset", "marker1High",
                            "marker1Low", "marker2High", "marker2Low"]
...
""")
    ...
    def saveWaveform(self):
        ...
        if 'waveform' in gv and 'markers' in gv:
            ...
            markers = gv['markers']
            wavedata = self.panel.instrument.writeRealData(
                numpy.array(waveform), numpy.array(markers))
            self.panel.instrument.createWaveform(
                str(self.waveformname.text()), wavedata, 'REAL')
...
class Panel(FrontPanel):
        ...
        # Update handler. Gets invoked whenever the instrument gets a new
        # trace.
    ...
    def updateGraph(self):
        ...
        self.waveformGraph.axes.plot(
            range(0, len(waveform._data)), waveform._data)
    ...
    def __init__(self, instrument, parent=None):
        ...
        self.connect(self.GpibCommand, SIGNAL(
            "returnPressed()"), self.evaluateGpibCommand)
        self.connect(GpibSubmit, SIGNAL("clicked()"), self.evaluateGpibCommand)
        self.connect(self.waveformlist, SIGNAL(
            "itemSelectionChanged()"), self.updateGraph)
