...
class Instr(VisaInstrument):
    ...
    def offset(self):
        o = float(
            self.ask("SOURCE%d:VOLTAGE:LEVEL:IMMEDIATE:OFFSET?" % (self._source)))
        ...
        for i in range(1, len(points)):
            ...
            for j in range(start[0], end[0] + 1):
                if j > 0 and j < len(waveform):
                    waveform[j] = start[
                        1] + int(float((end[1] - start[1]) * (j - start[0])) / float(end[0] - start[0]))
            start = end
    ...
    def _encodeWaveform(self, waveform, fastMethod=True):
        if fastMethod:
            ...
            data[:] = waveform[:]
            ptr = data.ctypes.data_as(ctypes.POINTER(ctypes.c_char))[
                :len(waveform) * 2]
            return ptr
        data = ""
    ...
    def writeWaveform(self, name, waveform):
        ...
        self.write("DATA:DATA EMemory,#%d%d%s" %
                   (len(str(len(waveform) * 2)), len(waveform) * 2, data))
    ...
    def setLow(self, o):
        self.write("SOURCE%d:VOLTAGE:LEVEL:IMMEDIATE:LOW %f V" %
                   (self._source, o))
        return self.low()

    def low(self):
        l = float(self.ask("SOURCE%d:VOLTAGE:LEVEL:IMMEDIATE:LOW?" %
                           (self._source)))
        self.notify("low", l)
        self._params['low'] = l
        return l
    ...
    def high(self):
        h = float(self.ask("SOURCE%d:VOLTAGE:LEVEL:IMMEDIATE:HIGH?" %
                           (self._source)))
        self.notify("high", h)
        self._params['high'] = h
        return h
    ...
    def setHigh(self, a):
        self.write("SOURCE%d:VOLTAGE:LEVEL:IMMEDIATE:HIGH %f V" %
                   (self._source, a))
        return self.high()

    def amplitude(self):
        a = float(
            self.ask("SOURCE%d:VOLTAGE:LEVEL:IMMEDIATE:AMPLITUDE?" % (self._source)))
    ...
    def period(self):
        p = 1.0 / float(self.ask("SOURCE%d:FREQUENCY:FIXED?" %
                                 self._source)) * 1e9
    ...
    def initialize(self, visaAddress="TCPIP0::192.168.0.21::inst0", source=1):
        try:
            self._params = dict()
            self.waveforms = []
            self._source = source
