...
class Panel(FrontPanel):
    ...
    def __init__(self, instrument, parent=None):
        ...
        self.connect(self.spectro.start, SIGNAL(
            "clicked()"), self.measureSpectro)
        self.connect(self.rabi.start, SIGNAL("clicked()"), self.measureRabi)
        self.connect(self.t1.start, SIGNAL("clicked()"), self.measuret1)
        self.connect(self.sCurves.start, SIGNAL(
            "clicked()"), self.measureSCurves)
        self.connect(self.stopButton, SIGNAL("clicked()"), self.stop)

    def measureSpectro(self):
        self.instrument.dispatch("measureSpectroscopy", float(self.spectro.startEdit.text()), float(
            self.spectro.stopEdit.text()), float(self.spectro.stepEdit.text()), float(self.spectro.powerEdit.text()))

    def measureRabi(self):
        self.instrument.dispatch("measureRabi", float(self.rabi.startEdit.text()), float(
            self.rabi.stopEdit.text()), float(self.rabi.stepEdit.text()), float(self.rabi.powerEdit.text()))

    def measuret1(self):
        self.instrument.dispatch("measureT1", float(self.t1.startEdit.text()), float(
            self.t1.stopEdit.text()), float(self.t1.stepEdit.text()), self.t1.accurateEdit.isChecked())
