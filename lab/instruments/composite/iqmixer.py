...
class Instr(Instrument):
  ...
  def initialize(self, MWSource, AWG, AWGChannels, fsp,offsetCube=None,sidebandCube=None):
    ...
    """
    instrumentManager=InstrumentManager() # Bad programming: an instrument should not be able to start an instrument manager
    ...
    self._params["dirName"]=None        # directory name for offset and sideband calibration files
    ...
    self._topOptiCube=None              # datacube for storing a set of minimization process
    self._optiCube=None                 # datacube for storing a single minimization process
  ...
  def setCal(self,offsetCalCube=None,sidebandCalCube=None):
    ...
    for calCube,newCalCube in zip(['_offsetCalCube','_sidebandCalCube'],[offsetCalCube,sidebandCalCube]):
      if isinstance(newCalCube,Datacube): self.set_attr(calCube,newCalCube)
  ...
  def saveCal(self,directory=None):
    ...
    if directory is None or not os.path.isdir(directory):
      try:
        ...
        if not os.path.isdir(directory): raise
      except:
        directory=os.getcwd()
    for cube,suffixName in zip([self._offsetCalCube,self._sidebandCalCube],suffixNames):
      if isinstance(cube,Datacube):
        ...
        if register is not None:
          key=self._name+'_Cal'
          self.debugPrint('storing register key ',key,' = ',directory)
  ...
  def loadCal(self,directory=None,create=False):
    ...
    for calCube,suffixName in zip(['_offsetCalCube','_sidebandCalCube'],suffixNames):
      ...
      try:                                        # 1) try to load from files,
        ...
        if create:                                # 2) or create an empty datacube if allowed.
  ...
  def calToDataManager(self):
    ...
    """
    if isinstance(self._offsetCalCube,Datacube):   self._offsetCalCube.toDataManager()
    if isinstance(self._sidebandCalCube,Datacube): self._sidebandCalCube.toDataManager()
  ...
  def generateSidebandWaveform(self, IF=0, c=None, phi=None, length=None, delay=0, normalize=True,useCalibIfNone=True,LO=None):
    ...
    """
    if length is None : length= self._params["AWGMaxPoints"]
    ...
    if any([c,phi]) is None:
      ...
      if useCalibIfNone:                            # get corrections from calibration
        try:
          if LO is None:  LO=self._mwg.frequency()    # as corrections depend on LO, get it if not provided
          self.debugPrint('calling sidebandCorrection with LO = ',LO,' and IF = ',IF)
          c1,phi1=self.sidebandCorrection(LO,IF)
        except Exception as e:
          print e
      if c is None: c=c1
      if phi is None: phi=phi1
    self.debugPrint('using corrections c,phi = ',c,', ',phi)
    # see A. Dewes' thesis page 87
    # C,PHI divided by 20 so that first increment of 1 represents 1/20=5% of amplitude and 3deg
    cr = c*exp(1j*phi)
  ...
  def loadSidebandWaveform(self, IF=0, c=None, phi=None,length=None,useCalibIfNone=True):
    ...
    waveform = self.generateSidebandWaveform(IF = IF, c = c, phi = phi, useCalibIfNone=useCalibIfNone, length = length)
    self._awg.loadiqWaveform(waveform,channels=self._params["AWGChannels"],waveformNames=["SBCalI-"+str(channels[0]),"SBCalQ-"+str(channels[1])])
    return waveform
  ...
  def setSidebandWaveform(self):
    ...
    self._awg.setWaveform(channels[1],"SBCalQ-"+str(channels[1])) # PROBABLY USELESS
  ...
  def calibrate(self,IForIFRange,offsetOnly=False,optToDataMan=False,calToDataMan=False):
    ...
    """
    IFRange=IForIFRange
    if not hasattr(IForIFRange, "__iter__"): IFRange=[IForIFRange] 
    print 'calibration in progress'
    self.calibrateIQOffset(optToDataMan=optToDataMan,calToDataMan=calToDataMan)
    if not offsetOnly:
      self.calibrateSideband(IFRange=IFRange,optToDataMan=optToDataMan,calToDataMan=calToDataMan)
  ...
  def calibrateIQOffset(self,LORange = None,reference = -5, method="scipy.optimize.fmin",save=True,optToDataMan=False,calToDataMan=False):
    ...
    """
    self.debugPrint('in calibrateIQOffset with LORange, save, optToDataMan,calToDataMan = ',(LORange,save,optToDataMan,calToDataMan))
    if LORange is None: LORange=[self._mwg.frequency()]         # use current carrier frequency if not specified 
    elif isinstance(LORange,[float, int]): LORange=[LORange]  # make a list if single frequency 
    channels=self._params["AWGChannels"]
    try:
      self.setup(reference=reference)                          # setup the different generators
      if not self._offsetCalCube: self._offsetCalCube=Datacube(name=self._name+suffixNames[0])
      cube =self._offsetCalCube
      self.dictCube(cube)
      if calToDataMan: cube.toDataManager()
      ...
      if optToDataMan: cu.toDataManager()
      LOpower=self._mwg.power()
      for LO in LORange:                                      # loop over LO frequencies
        cu.set(LO_dBm=LOpower,LO_GHz=LO,columnOrder=['LO_dBm','LO_GHz'],commit=True)
        ...
        if  method == "scipy.optimize.fmin":                    # find minimum power
          (voltages,minimum) = self.optimizeIQMixerPowell(replaceOptiCube=False)
        elif method == "vs.fmin":
          (voltages,minimum) = self.optimizeIQMixerVS(replaceOptiCube=False)
        else:
        ...
        try:                                                  # remove outdated offset at the same frequency
          ...
          print "search or remove error"                      # and store in the offset cube
        cube.set(LO_dBm=LOpower, LO_GHz=LO, lowI=voltages[0], lowQ=voltages[1], RF_dBm = minimum,columnOrder=['LO_dBm','LO_GHz','lowI','lowQ','RF_dBm'],commit=True)
        cube.sortBy("LO_GHz")                                 # sort in frequency
        if save : cube.savetxt(overwrite=True)                # and save in file if requested
    except:
      raise
    finally:                                                  
      self.teardown()                                         # restore all generator
      self.updateOffsetCalInterpolation()                     # update interpolation function
      LO=self._mwg.frequency()                         
      self._awg.setOffset(channels[0],self.iOffset(LO))       # set the I and Q offsets on the AWG
  ...
  def calibrateSideband(self,LORange = None,IFRange = arange(-0.5,0.51,0.1),reference =0,method="scipy.optimize.fmin",save=True,optToDataMan=False,calToDataMan=False):
    ...
    """
    self.debugPrint('in calibrateSideband with LORange, IFRange, save, optToDataMan,calToDataMan = ',(LORange,IFRange,save,optToDataMan,calToDataMan))
    if LORange is None:  LORange=[self._mwg.frequency()]
    elif isinstance(LORange,[float, int]): LORange=[LORange]
    try:
      self.setup(reference = reference)                       # setup the different generators
      if not isinstance(self._sidebandCalCube,Datacube):  self._sidebandCalCube=Datacube(name=self._name+'_SBCal')
      cube=self._sidebandCalCube                              # this is the final calibration cube
      if calToDataMan: cube.toDataManager()
      self.dictCube(cube)
      self._mwg.turnOn()
      channels = self._params["AWGChannels"]
      self.setSidebandWaveform()                              # probably useless call since successive waveforms will be loaded later by loadSidebandWaveform                            
      self._awg.runAWG()
      self._awg.setState(channels,[True]*len(channels))       # start requested channels
      cu=self._topOptiCube=Datacube(name=self._name+'_SBOptim')  # this is the optimization cube for control     
      if optToDataMan: cu.toDataManager()
      for LO in LORange:                                      # loop over LO frequencies
        LO = round(LO,3)                                      # center frequency rounded to 1 MHz
        self._mwg.setFrequency(LO)
        ...
        self._awg.setOffset(channels[1],self.qOffset(LO))
        cu.set(LO_dBm=self._mwg.power(),LO_GHz = LO,columnOrder=['LO_dBm','LO_GHz'],commit=True)
        if cube.column('LO_GHz') is None or LO not in cube.column('LO_GHz'):
          cube.set(LO_dBm=self._mwg.power(),LO_GHz = LO,columnOrder=['LO_dBm','LO_GHz'],commit=True)
          child = Datacube("LO_GHz=%g" % LO)
          cube.addChild(child, LO_GHz=LO)
        else:
          child=cube.children(LO_GHz=LO)[-1]
        cuChild=Datacube("LO_GHz=%g" % LO)                     # child cube at a particular LO carrier LO to gather grand children cubes
        cu.addChild(cuChild)
        for IF in IFRange:                                     # loop over IF frequencies
          cuChild.set(IF_GHz = IF,commit=True)
          self._optiCube=Datacube("IF_GHz=%g" % IF)            # optimization grandchild cube at a particular couple (LO,IF)
          cuChild.addChild(self._optiCube)
          print "LO_GHz=%g, IF_GHz=%g" % (LO,IF)
          self._fsp.write("SENSE1:FREQUENCY:CENTER %f GHZ" % (LO-IF))
          time.sleep(1)
          print "minimizing"
          if  method == "scipy.optimize.fmin":
            direc = eye(2, dtype=float)*0.1                     # defines the directions, as well as the initial steps (1 means 100% correction and 1 radian)
            result = scipy.optimize.fmin_powell(lambda x,*args: self.measureSidebandPower(x,*args),[0,0],args = [IF],direc=direc,full_output = 1,xtol = 0.001,ftol = 1e-2,maxiter =50,maxfun =50, disp=True, retall=True)
          elif method == "vs.fmin":
            minimizer=Minimizer(lambda x: self.measureSidebandPower(x,IF_GHz=IF),[0.,0.],[[-0.2,0.2],[-2,2]],xtol=[0.01,0.01],maxfun=20,maxiter=3)
            minimizer.minimize()
          ...
          self.loadSidebandWaveform(IF=IF, c=params[0], phi=params[1],length=self._params["AWGMaxPoints"])
          child.set(LO_GHz=LO, IF_GHz=IF, absC=params[0], argC_rad=params[1], columnOrder=['LO_GHz','IF_GHz','absC','argC_rad'])
          for i in [-3,-2,-1,0,1,2,3]:                            # measure the different sidebands
            self._fsp.write("SENSE1:FREQUENCY:CENTER %f GHZ" % (LO+IF*i))
            time.sleep(1)
            if i < 0:
              suppl = "m"
            else:
          ...
          child.commit()
        if save:  cube.savetxt(overwrite=True)
    except:
      raise
    finally:
      self.teardown()
  ...
  def setup(self,averaging = 10,reference = -30):
    ...
    """
    self.debugPrint('in setup with averaging,reference = ',(averaging,reference))
    self._fsp.storeConfig("BeforeIQCal")                # store previous configurations of fsp,awg for IF, and mwg for LO
  ...
  def memorizeAWGOffsets(self):
    ...
    """
    self._awgOffsets=[self._awg.offset(i+1) for i in range(self._awg.channels()) ]
  ...
  def setupAWG(self):
    ...
    self._awg.createRawWaveform("zeroes",(waveformOffset+1.0)/2.0*((1<<14)-1),self._markers,"INT")
  ...
  def optimizeIQMixerVS(self,replaceOptiCube=True):
    if replaceOptiCube or not isinstance(self._optiCube,Datacube): self._optiCube=Datacube(name='IQMixerVS')
  ...
  def optimizeIQMixerPowell(self,x0Limit=.1,replaceOptiCube=True):
    ...
    """
    if replaceOptiCube or not isinstance(self._optiCube,Datacube): self._optiCube=Datacube(name='IQMixerPowell')
  ...
  def measurePower(self,lows):
    ...
    self._optiCube.set(I_V=lows[0],Q_V=lows[1],RF_dBm=power,columnOrder=['I_V','Q_V','RF_dBm'],commit=True)
  ...
  def measureSidebandPower(self,x,IF):
    ...
    #if abs(phi)>3:
      #return 100
    self.loadSidebandWaveform(IF = IF,c = c,phi = phi, length=self._params["AWGMaxPoints"])
    time.sleep(0.5)
    power = self.measureAveragePower()
    self._optiCube.set(absC=c,argC_rad=phi,RF_dBm=power,columnOrder=['absC','argC_rad','RF_dBm'],commit=True)
  ...
  def updateOffsetCalInterpolation(self):
    ...
    if cube and 'LO_GHz' in cube.names():
      if len(cube.column('LO_GHz'))>1:
        freqs,iOffsets,qOffsets = cube.column('LO_GHz'),cube.column('lowI'),cube.column('lowQ')
        self._iOffsetInterpolation = scipy.interpolate.interp1d(freqs,iOffsets)        
        self._qOffsetInterpolation = scipy.interpolate.interp1d(freqs,qOffsets)
      ...
      if rows == []: 
        try:
          if len(calibrationData['IF_GHz'])>1:
            ...
            self.debugPrint('absC', cList, 'argC_rad', phiList, 'IF_GHz',IFList)
            try:
              self.debugPrint("trying to interpolate")
              phiInterpolation = scipy.interpolate.interp1d(IFList,phiList)      
              cInterpolation = scipy.interpolate.interp1d(IFList,cList)      
              c = cInterpolation(IF)
        ...
        c,phi = calibrationData.column("absC")[rows[-1]], calibrationData.column("argC_rad")[rows[-1]]
      else:
        c,phi = calibrationData.column("absC")[-1], calibrationData.column("argC_rad")[-1]
