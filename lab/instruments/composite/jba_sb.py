...
class Instr(Instrument):
  ...
  def generateShape(self):
    ...
    self.shape[self._shapeParams["startingTime"]+self._shapeParams["offsetDelay"]:self._shapeParams["startingTime"]+self._shapeParams["offsetDelay"]+self._shapeParams["risingTime"]]=linspace(0,1,self._shapeParams["risingTime"])
    self.shape[self._shapeParams["startingTime"]+self._shapeParams["offsetDelay"]+self._shapeParams["risingTime"]:self._shapeParams["startingTime"]+self._shapeParams["offsetDelay"]+self._shapeParams["risingTime"]+self._shapeParams["plateauLength"]]=1
    self.shape[self._shapeParams["startingTime"]+self._shapeParams["offsetDelay"]+self._shapeParams["risingTime"]+self._shapeParams["plateauLength"]:self._shapeParams["startingTime"]+self._shapeParams["offsetDelay"]+self._shapeParams["risingTime"]+self._shapeParams["plateauLength"]+self._shapeParams["risingTime"]]=linspace(1,self._shapeParams["plateau"],self._shapeParams["risingTime"])
    self.shape[self._shapeParams["startingTime"]+self._shapeParams["offsetDelay"]+self._shapeParams["risingTime"]+self._shapeParams["plateauLength"]+self._shapeParams["risingTime"]:self._shapeParams["startingTime"]+self._shapeParams["offsetDelay"]+self._shapeParams["risingTime"]+self._shapeParams["plateauLength"]+self._shapeParams["risingTime"]+self._shapeParams["latchLength"]]=self._shapeParams["plateau"]
...
#    self._pulseGenerator.generatePulse(duration=duration, frequency=self._frequency, amplitude=self._amplitude, DelayFromZero=delayFromZero,useCalibration=True, phase=phase,shape=amplitude*shape, name=self.name())
    ...
    self._pulseGenerator.addPulse(generatorFunction="arb",overwrite=True, frequency=self._frequency, applyCorrections=True,shape=self.shape*amplitude, name=self.name())
  ...
  def setAmplitude(self, amplitude, magnitudeButton=None, phase=None,**args):
    ...
    """
    if magnitudeButton is None: magnitudeButton=self._magnitudeButton
    if magnitudeButton == 'formAmplitude':
        if phase is None:
          phase=self._phase
        self.phase=phase
        if abs(amplitude)>1.2: amplitude=amplitude/abs(amplitude)*1.2
        self.setFrequency(frequency=self._frequency,amplitude=amplitude,phase=phase)
        #self.sendWaveform()
        time.sleep(1.)
...
#      voltages=linspace(bounds[0],bounds[1],200)
  ...
  def measure(self,nLoops=None,fast=False):
    ...
    """
    try:
      if nLoops is None:  nLoops=self.nLoops()
