...
class Instr(Instrument):
    ...
    def reInitCalibration(self):
        ...
        """
        register["%s OffsetCal" %
                 self._name] = self._calibration.initCalibrationData()
        self._calibration = OffsetOptimization(
            fsp=self._fsp, awg=self._AWG, mwg=self._MWSource, channels=self._params["AWGChannel"])
    ...
    def initialize(self, MWSource, AWG, AWGChannel, fsp):
        ...
        try:
            ...
            self._calibration._offsetCalibrationData.loadtxt(
                register["%s OffsetCal" % self._name], loadChildren=True)
        except:
