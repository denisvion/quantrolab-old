...
class Panel(FrontPanel):
    ...
    def __init__(self,instrument,parent=None):
        ...
        self.connect(self.calibrateOffsetButton,SIGNAL("clicked()"),self.calibrateIQOffset)
        ...
        self.connect(self.calToDataManCheckBox,SIGNAL("stateChanged(int)"),self.calToDataManager)
        self.connect(self.optToDataManCheckBox,SIGNAL("stateChanged(int)"),self.optToDataManager)
    ...
    def saveState(self):
        ...
        directory = str(QFileDialog.getExistingDirectory (caption ='Select folder for saving calibration files',options=QFileDialog.DontUseNativeDialog,directory = QString(self.workingDirectory())))
        if os.path.isdir(directory):
            self.setWorkingDirectory(directory)
            self.instrument.dispatch("saveState",directory)

    def restoreState(self):
        directory = str(QFileDialog.getExistingDirectory (caption ='Select folder with calibration files',directory=QString(self.workingDirectory())))
        if os.path.isdir(directory):
            self.setWorkingDirectory(directory)
    ...
    def calibrateIQOffset(self):
        self.disableButtons()
        self.instrument.dispatch("calibrateIQOffset",calToDataMan=self.calToDataManCheckBox.isChecked(),optToDataMan=self.optToDataManCheckBox.isChecked())     

    def calibrate(self):
        self.disableButtons()
        self.instrument.dispatch("calibrate",self.fsbEdit.getValue(),calToDataMan=self.calToDataManCheckBox.isChecked(),optToDataMan=self.optToDataManCheckBox.isChecked())
    ...
    def testSideband(self):
        self.instrument.dispatch("loadSidebandWaveform",IF=self.fsbEdit2.getValue())
