...
#    self.StopButton.setEnabled(True)
    ...
    def adjustSwitchingLevel(self):
        ...
        self.instrument.dispatch("adjustSwitchingLevel", float(
            self.levelEdit.text()), float(self.accuracyEdit.text()))
    ...
    def calibrate(self):
        ...
        self.instrument.dispatch("calibrate", level=float(
            self.levelEdit.text()), accuracy=float(self.accuracyEdit.text()))
        # self.calibrateButton.setEnabled(False)
...
#    self.histograms.axes.hist(trends[1],normed = True,bins = 30)
        ...
        if self.squareIQ.isChecked():
            squareSpace(self.iq.axes)
...
# Utility functions
    ...
    def squareSpace(axes):
        w = 1.25 * max(map(abs, axes.axis()))
        axes.axis([-w, w, -w, w])
