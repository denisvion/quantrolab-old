...
class Instr(Instrument):
    ...
    def saveState(self, name):
        ...
        state["readoutWaveform"] = (
            copy.deepcopy(self._readoutWaveform)).tolist()
        return state
...
#    self.loadReadoutWaveform(self._params["measureTime"],self._params["riseTime"],self._params["fallTime"],self._params["latchTime"],self._params["latchHeight"])
    ...
    def loadReadoutWaveform(self, measureTime=200, riseTime=20, fallTime=30, latchTime=2000, starkPulseLength=0, starkPulseHeight=0, starkPulseDelay=0, scale=1.0, latchHeight=0.8, acquisitionTime=250, waitTime=0, fallTime2=10, safetyMargin=100):
        waveformLength = 1 + (waitTime + measureTime + latchTime + riseTime +
                              fallTime + fallTime2 + starkPulseLength + starkPulseDelay) * 2
        fullWaveform = zeros((waveformLength), dtype=uint16)
        waveform = fullWaveform[
            (starkPulseLength + starkPulseDelay + waitTime) * 2:]
        fullWaveform[1 + waitTime * 2:(waitTime + starkPulseLength)
                     * 2.0] = starkPulseHeight * ((1 << 14) - 1)
        waveform[1:riseTime * 2 +
                 1] = linspace(0, (1 << 14) - 1, riseTime * 2) * scale
        waveform[1 + riseTime * 2:measureTime * 2 +
                 1 + riseTime * 2] = ((1 << 14) - 1) * scale
        waveform[measureTime * 2 + 1 + riseTime * 2:measureTime * 2 + 1 + riseTime * 2 + fallTime *
                 2] = linspace((1 << 14) - 1, ((1 << 14) - 1) * latchHeight, fallTime * 2) * scale
        waveform[measureTime * 2 + 1 + riseTime * 2 + fallTime * 2:-
                 fallTime * 2] = int((float(1 << 14) - 1) * latchHeight) * scale
        waveform[-fallTime * 2:] = linspace(
            int((float(1 << 14) - 1) * latchHeight), 0, fallTime * 2) * scale
        ...
        self._params["readoutWindowStart"] = self._params["internalDelay"] + self._params["returnDelay"] + riseTime + \
            measureTime + fallTime + \
            self._params["safetyMargin"] + \
            starkPulseLength + waitTime + starkPulseDelay
        self._readoutWaveform = fullWaveform
        self.notify("readoutWaveform", fullWaveform)
        self._acqiris.ConfigureV2(
            **{"delayTime": (self._params["readoutWindowStart"]) * 1e-9})

    def updateReadoutWaveform(self):
        self._readoutWaveform = self._afg.readWaveform(
            self._params["waveform"])
    ...
    def generatePulse(self, holdTime=200, sampleTime=2000, delay=10000, holdHeight=0.9, totalLength=20000):
        ...
        pulse[delay + holdTime:delay + holdTime +
              sampleTime] = int(255 * holdHeight)
    ...
    def measureSCurve(self, voltages=None, ntimes=40, microwaveOff=True):
        ...
        try:
            ...
            if voltages is None:
                ...
                (vmin, vmax) = getVoltageBounds(
                    v0, self, self._params["variable"], ntimes)
                voltages = arange(vmin, vmax, 0.005)
                self.notify(
                    "status", "Measuring S curve in voltage range  [%g - %g]..." % (vmin, vmax))
            for v in voltages:
                self.setVoltage(v)
                ...
                data.commit()
                self.notify("sCurve", (data.column("v"),
                                       data.column(self._params["variable"])))
        finally:
    ...
    # Calibrates the Qubit readout and adjust the switching probability with a
    # given accuracy.
    def calibrate(self, level=0.1, accuracy=0.025, microwaveOff=True):
        try:
            ...
            self.notify("status", "Calibration complete. Measure of separation: %g" %
                        self.separationMeasure(acquire=False))
    ...
    def _attenuatorRangeCheck(self, voltages):
        ...
        for i in range(0, len(voltages)):
            ...
            varsum = cov(trends[self._params["acqirisChannel"]]) + \
                cov(trends[self._params["acqirisChannel"] + 1])
            data.set(v=v)
    ...
    def switchingProbability(self):
        ...
        p = self._acqiris.probabilities()[self._params[
            "acqirisChannel"] / 2, 0]
        return p
    ...
    def separationMeasure(self, acquire=True):
        ...
        trends = self._acqiris.trends()
        angle = self._acqiris.bifurcationMapRotation(
        )[self._params["acqirisChannel"] / 2]
        rotatedTrends = zeros((2, len(trends[self._params["acqirisChannel"]])))
        rotatedTrends[0, :] = trends[self._params["acqirisChannel"], :] * \
            cos(angle) + \
            trends[self._params["acqirisChannel"] + 1, :] * sin(angle)
        left = []
        right = []
    ...
    def acquire(self, ntimes=20):
        ...
        rotatedTrends[0, :] = trends[self._params["acqirisChannel"], :] * \
            cos(angle) + \
            trends[self._params["acqirisChannel"] + 1, :] * sin(angle)
        rotatedTrends[1, :] = -trends[self._params["acqirisChannel"], :] * \
            sin(angle) + \
            trends[self._params["acqirisChannel"] + 1, :] * cos(angle)
    ...
    def adjustSwitchingLevel(self, level=0.2, accuracy=0.05, verbose=False, minSensitivity=15.0, microwaveOff=True, nmax=100):
        try:
            ...
            # If the working point is not good, we search for a good one by
            # ramping the voltage in the attenuator.
            ...
            while fabs(p - level) > accuracy:
                if self.stopped():
                    raise Exception("Got stopped!")
                self.notify(
                    "status", "Switching probability at %g V: %g" % (v, p))
                cnt += 1
                diff = p - level
                if verbose:
                    print "v = %g, p = %g, dp = %f" % (v, p, diff)
                # If switching probability is too high, we increase the
                # voltage. If it is too low, we decrease it...
    ...
    def adjustSwitchingLevel2(self, level=0.2, accuracy=0.05, verbose=False, minSensitivity=15.0, microwaveOff=True, nmax=100, checkBivaluated=True):
        try:
            ...
            if microwaveOff:
                self._qubitmwg.turnOff()
            vOld = self._attenuator.voltage()
            # First check that the distribution is bivaluated with a sufficient
            # gap between the 2 values (with separationMeasure().
            ...
            while fabs(pOld - level) > accuracy:
                if self.stopped():
                    raise Exception("Got stopped!")
                # increase the increment until dp is larger than twice both the
                # variance and the target accuracy
                dp = 0
                dv2 = 0
                ...
                if fabs(pOld - level) > accuracy:
                    ...
                    if v == vOld:
                        raise Exception(
                            "Twice the same attenuation voltage v although target was not reached!")
                    vOld = v
                    pOld = p
                    cnt += 1
                    if cnt > nmax:
                ...
                vOLd = self._attenuator.voltage()
                pOld = self.switchingProbability()
            self.notify("status", "Switching probability: %g %%" %
                        (pOld * 100))
            return (p, cnt)
        finally:
            if status == True:
    ...
    def adjustRotationAndOffset(self):
        try:
            ...
            self._acqiris.ConfigureChannel(
                self._params["acqirisChannel"] + 1, offset=0)
            self._acqiris.ConfigureChannel(
                self._params["acqirisChannel"] + 2, offset=0)
            self._acqiris.setBifurcationMapRotation(
                self._params["acqirisChannel"] / 2, 0)
            cnt = 0
            ...
            while cnt < 3:                  # repeat for higher precision
                ...
                trends = self._acqiris.trends()
                means = mean(trends[self._params["acqirisChannel"]:self._params[
                             "acqirisChannel"] + 2, :], axis=1)
                covar = var(trends[self._params["acqirisChannel"]                                   :self._params["acqirisChannel"] + 2, :])
                offsets[0] -= means[0]
                offsets[1] -= means[1]
                self.notify("iqdata", trends)
                self._acqiris.ConfigureChannel(
                    self._params["acqirisChannel"] + 1, offset=offsets[0])
                self._acqiris.ConfigureChannel(
                    self._params["acqirisChannel"] + 2, offset=offsets[1])
                print covar
                print means
            covar = cov(trends[self._params["acqirisChannel"]] - means[0],
                        trends[self._params["acqirisChannel"] + 1] - means[1])
            ...
            self._acqiris.setBifurcationMapRotation(
                self._params["acqirisChannel"] / 2, angle)
            pBefore = self._acqiris.probabilities(
            )[self._params["acqirisChannel"] / 2, 0]
            ...
            pAfter = self._acqiris.probabilities(
            )[self._params["acqirisChannel"] / 2, 0]
            if pAfter > pBefore:
                angle += math.pi
                self._acqiris.setBifurcationMapRotation(
            ...
            self.trends[0, :] = trends[self._params["acqirisChannel"], :] * \
                cos(angle) + \
                trends[self._params["acqirisChannel"] + 1, :] * sin(angle)
            self.trends[1, :] = -trends[self._params["acqirisChannel"], :] * \
                sin(angle) + \
                trends[self._params["acqirisChannel"] + 1, :] * cos(angle)
        ...
        try:
            ...
            while cnt < 3:                  # repeat for higher precision
                ...
                self._acqiris.ConfigureChannel(
                    self._params["acqirisChannel"] + 2, offset=offsets[1])
                print means
            angle = self_.acqiris.parameters()["rotation"][
                self._params["acqirisChannel"]]
            # We calculate the covariance matrix.
            covar = cov(trends[self._params["acqirisChannel"]] - means[0],
                        trends[self._params["acqirisChannel"] + 1] - means[1])
            # We calculate the eigenvalues and eigenvectors of the covariance
            # matrix.
            la, v = linalg.eig(covar)
            # We calculate the rotation angle from the eigenvector matrix M =
            # [(cos(theta),-sin(theta)),(sin(theta),cos(theta))], using the
            # eigenvector of maximum weight (max eigenvalues)
            if la[1] > la[0]:
                angle += math.atan2(v[0, 0], v[0, 1])
            else:
                angle += math.atan2(v[1, 0], v[0, 0])
    ...
    def initialize(self, **kwargs):
        ...
        defaultParams = {'acqirisChannel': 0, 'variable': "p1x", 'muwave': "cavity1mwg", 'attenuator': "AttS2", 'acqiris': "acqiris", 'qubitmwg': "qubit1mwg", 'polarity': 1,
                         'afg': "afg3", 'waveform': "USER1", 'internalDelay': 243, 'returnDelay': 57, 'acquisitionTime': 400, 'sampleInterval': 3e-9, 'trigSlope': 0, 'safetyMargin': 100}
