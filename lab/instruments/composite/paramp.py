...
import os,sys,getopt,re,struct,pickle,math,numpy,scipy,scipy.optimize,scipy.interpolate
...
# Class definition
initialInstruments={'biasGen':None,'pumpGen':None,'testVNA':None,'testGen':None,'testFSA':None}

defaultBiasMethodNames={'setBias':'setVoltage','bias':'voltage'}
defaultPumpMethodNames={'setPumpFreq':'setFrequency','pumpFreq':'frequency','setPumpPow':'setPower','pumpPow':'power','setPumpPhase':'setPhase','pumpPhase':'phase','setPumpOutput':'setOutput','pumpOutput':'output'}
...
defaultTestGenMethodNames={'setTestGenFreq':'setFrequency','testGenFreq':'frequency','setTestGenPow':'setPower','testGenPow':'power','setTestGenOutput':'setOutput','testGenOutput':'output'}
defaultFSAMehtodNames={'setFSAFreq':'setCenterInGHz','FSAFreq':'getCenterInGHz','setFSASpan':'setSpanInGHz','FSASpan':'getSpanInGHz','setFSABandwidth':'setResBWInHz','FSABandwidth':'getResBWInHz','FSATrace':'getSingleTrace'}
defaultMethodNames={}
for d in [defaultBiasMethodNames, defaultVNAMethodNames, defaultFSAMehtodNames]: defaultMethodNames.update(d)

initialMethods=dict()
for key in defaultMethodNames.keys(): initialMethods[key]=None
...
class Instr(Instrument):
  ...
  def initialize(self,biasGen=None,pumpGen=None,testVNA=None,testFSA=None):
    ...
    self._manager=None
    try:
      self._manager=InstrumentManager()   # has now a handle to the singleton instrument manager.    
    except:
  ...
  def setSubInstrum(self,name,instrum,defaultMethods):
    ...
    """
    if instrum is None: return
    inst,instDict=instrum                                           # separate reference to sub-instrument from dictionnary of sub-instrument methods
    print inst,insDict
    if inst is not None:                                                  # do not do anything if sub-instrument is None
      if isinstance(inst,str):                                      # instr is a string => it's the name of the sub-instrument => retrieve it from the instrument manager
        try:
          inst=self._manager.getInstrument(inst)
        except:
          print 'instrument '+inst+' not found.'
          raise 
      else:                                                         # otherwise assume that inst is the sub-instrument object
        inst=instr
      self._subInstruments[name]=inst                               # save the sub-instrument object in the sub-instrument dictionnary
      
      for methodKey in defaultMethods.keys():                       # for all expected methods of sub-instrument
        methodName=None
        ...
        if methodName is not None:
          self._methods[methodKey]=getattr(inst,methodName)        # memorize the methods in the field 'biasGen' of self._subInstruments dictionary
          setattr(self,methodKey,getattr(inst,methodName))         # self.methodkey aliases now the method inst.methodName
  ...
  def _missing(self,instrumentList,methodList):
    ...
    """
    missing=any([self._subInstruments[instrumentName] is None for instrumentName in instrumentList ]) or not all([hasattr(self,methodName) for methodName in methodList])
    if missing: print 'Missing instrument or method.'
    return missing
  ...
  def calibrateFreq(self,biasStart,biasStop,biasStep=1,VNACenter=None,VNASpan=None,VNABandwidth=None):
    ...
    if missing: return
    self.setupVNA(VNACenter=VNACenter,VNASpan=VNASpan,VNABandwidth=VNABandwidth)
    loop=SmartLoop(biasStart,biasStep,biasStop)
    cube=Datacube(self._name+'-FreqCal')
    cube.toDataManager()
    for bias in loop:
      self.setBias(bias,self._params["slewrateBias"])
      child=self.VNATrace(waitFullSweep=True, unwindPhase=True,phaseStart=180)
      cube.addChild(child)
      cube.set(bias=bias, freq=self.findFreq(child), columnOrder=['bias','freq'], commit=True)
  ...
  def calToDataManager(self):
    ...
    """
    if isinstance(self._freqOfBiasCube,Datacube):   self._freqOfBiasCube.toDataManager()
  ...
  def makeInterpBiasFreq(self,save=True):
    ...
    self._freqOfBias=scipy.interpolate.interp1d(cube['bias'], cube['freq'])
    i1,i2 = numpy.argmin(cube['freq']),numpy.argmax(cube['freq'])
    if i1 > i2: i1, i2 = i2 , i1
    try:
      self._biasOfFreq=scipy.interpolate.interp1d(cube['freq'][i1:i2], cube['bias'][i1:i2])
    except:
      print 'Error in inverse interpolation probably due to non-bijective behavior.'
    if save: self.saveState()

  def findFreq(self,child):
    i=indexLastPos=numpy.where(child['phaseCor']<0)[0][0]-1
    x0,x1,y0,y1=child['freq'][i],child['freq'][i+1],child['phaseCor'][i],child['phaseCor'][i+1]
  ...
  def setupFSA(self,averaging=10,reference=-30):
    ...
    Setup fsa
    """
    noWay = not all ([hasattr(self,method) for method in ['testGenFreq','setFSAFreq','setFSASpan','setFSABandWidth']])
    if noway: return
    bw=2000
    self.debugPrint('in setupFSA')
    self._fsp.storeConfig("BeforeParampCharac")                # store previous configurations of fsp,awg for IF, and mwg for LO
  ...
  def measureGainFromFSA(self):
    ...
    missing = not all ([hasattr(self,method) for method in ['testGenFreq','setFSAFreq','setFSASpan','setFSABandWidth']])
    if missing : return
