...
class Instr(Instrument):
    ...
    def measureT1(self, delays, data=None, t1Parameters=None, dataParameters=None):
        ...
        if duration == "rabi":
            duration = self._params['piRabiTime']

        if t1Parameters.has_key('remember'):
            if t1Parameters['remember']:
                self._params['t1Parameters'] = t1Parameters
        ...
        if data is None:
            ...
            if dataParameters['addToDataManager']:
                data.toDataManager()
        ...
        for delay in delays:
            ...
            self._pulseGenerator.generatePulse(
                duration=duration, frequency=frequency, DelayFromZero=10000 - delay - duration - offsetDelay, useCalibration=False)
    ...
    def measureRabi(self, durations, data=None, rabiParameters=None, dataParameters=None):
        ...
        if rabiParameters.has_key('remember'):
            if rabiParameters['remember']:
                self._params['rabiParameters'] = rabiParameters
        ...
        for duration in durations:
            self.clearPulses()
            self.generateRabiPulse(duration=duration, frequency=frequency,
                                   offsetDelay=offsetDelay, useCalibration=rabiParameters['useCalibration'])
            result = self._jba.measure(nLoops=nLoops, fast=True)[0]
            data.set(**result)
        ...
        try:
            ...
            [y0, dy, piRabi, t10], yfit = fitRabi(
                data['duration'], data[columnName], period=period)
            data.createColumn("%sfit" % columnName, yfit)
        except:
            print 'fit error'
            piRabi = 0
        if dataParameters['save']:
            data.savetxt()
    ...
    def measureRabiArea(self, areas, data=None, rabiParameters=None, dataParameters=None):
        ...
        for area in areas:
            ...
            self.clearPulses()
            self.generateRabiPulseArea(area=area, frequency=frequency, maxHeight=maxHeight,
                                       offsetDelay=offsetDelay, useCalibration=rabiParameters['useCalibration'], shape=shape)
            result = self._jba.measure(nLoops=nLoops, fast=True)[0]
            data.set(**result)
            data.set(area=area)
            data.commit()
    ...
    def measureSpectroscopy(self, frequencies, data=None, spectroscopyParameters=None, dataParameters=None):
        ...
        if spectroscopyParameters.has_key('remember'):
            self._params['spectroscopyParameters'] = spectroscopyParameters
        ...
        if spectroscopyParameters.has_key('power'):
            self._pulseGenerator._MWSource.setPower(
                spectroscopyParameters['power'])
        amplitude = spectroscopyParameters[
            'amplitude'] if spectroscopyParameters.has_key('amplitude') else 0
        offsetDelay = spectroscopyParameters['offsetDelay'] if spectroscopyParameters.has_key(
            'offsetDelay') else (self._params['offsetDelay'] if self._params.has_key('offsetDelay') else 20)
        ...
        for frequency in frequencies:
            ...
            # i.e. generate a rabi pulse on the AWG only the first time (no
            # need to re-generate an AWG pulse every time)
            if first:
                self._pulseGenerator.pulseList = []
                self.generateRabiPulse(duration=duration, amplitude=amplitude,
                                       offsetDelay=offsetDelay, useCalibration=useCalibration, frequency=frequency)
                first = False
        ...
        try:
            columnName = "b%i" % self._jba.bit
            [dy, f0, df, y0], yfit = fitLorentzian(
                data['frequency'], data[columnName], 1)
        ...
        if dataParameters['save']:
            data.savetxt()
        data.setParameters(Manager().saveState('currentState'))
    ...
    def generateRabiPulse(self, duration=None, amplitude=None, offsetDelay=None, useCalibration=False, clear=True, frequency=None):
        if frequency is None:
            frequency = self.frequency01()
        if duration is None:
            duration = self.rabiPeriod()
        if clear:
            self._pulseGenerator.pulseList = []
        if amplitude is None:
            amplitude = self.rabiAmplitude()
        if offsetDelay is None:
            offsetDelay = self.rabiOffset()

        self._pulseGenerator._MWSource.turnOn()
        self._pulseGenerator.generatePulse(duration=duration, amplitude=amplitude, frequency=frequency,
                                           DelayFromZero=10000 - duration - offsetDelay, useCalibration=useCalibration)
        self._pulseGenerator.sendPulse()
    ...
    def generateRabiPulseArea(self, area=None, maxHeight=None, offsetDelay=None, useCalibration=True, frequency=None, shape=None, clear=True, phase=0):
        if frequency is None:
            frequency = self.frequency01()
        if area is None:
            duration = self.rabiArea()
        if shape is None:
            shape = 'square'
        if maxHeight is None:
            maxHeight = self.rabiAmplitude()
        if offsetDelay is None:
            offsetDelay = self.rabiOffset()
        ...
        if clear:
            self._pulseGenerator.pulseList = []
        if shape == 'gaussian':
            self._pulseGenerator.addPulse(generatorFunction="gaussianSlopePulse", frequency=frequency, end=10000 - offsetDelay,
                                          sigma=sigma, maxHeight=maxHeight, cutOff=cutOff, area=area, applyCorrections=useCalibration, phase=phase)
        elif shape == 'square':
            self._pulseGenerator.addPulse(generatorFunction="square", frequency=frequency, amplitude=maxHeight, start=10000 -
                                          area / maxHeight - offsetDelay, stop=10000 - offsetDelay, applyCorrections=useCalibration, phase=phase)
        else:
            print "ERROR"
    ...
    def generateRabiPulseAreaDerivative(self, area=None, maxHeight=None, offsetDelay=None, useCalibration=False, frequency=None, shape=None, clear=True, phase=0, coefficient=1.):
        ...
        print sigma
        if maxHeight is None:
            maxHeight = self.gaussianParameter('maxHeight')
        cutOff = self.gaussianParameter('cutOff')

        if clear:
            self._pulseGenerator.pulseList = []
        if shape == 'gaussian':
            self._pulseGenerator.addPulse(generatorFunction="gaussianSlopePulseDerivative", frequency=frequency, end=10000 - offsetDelay,
                                          sigma=sigma, maxHeight=maxHeight, cutOff=cutOff, area=area, applyCorrections=useCalibration, phase=phase, coefficient=coefficient)
        elif shape == 'square':
            self._pulseGenerator.addPulse(generatorFunction="square", frequency=frequency, amplitude=maxHeight, start=10000 -
                                          area / maxHeight - offsetDelay, stop=10000 - offsetDelay, applyCorrections=useCalibration, phase=phase)
        else:
    ...
    def generateRotation(self, mag, angle, offsetDelay, clear=False, frequency=None):
        ...
        print "area =", area
        self.generateRabiPulseArea(area=area, offsetDelay=offsetDelay, useCalibration=True,
                                   shape='gaussian', clear=clear, phase=angle, frequency=frequency)
        self.generateRabiPulseAreaDerivative(area=area, offsetDelay=offsetDelay, useCalibration=True, shape='gaussian',
                                             clear=False, phase=angle - math.pi / 2, frequency=frequency, coefficient=self.dragCoef())
