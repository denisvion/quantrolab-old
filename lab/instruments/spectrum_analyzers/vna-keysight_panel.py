...
class Panel(FrontPanel):
    ...
    def __init__(self, instrument, parent=None):
        ...
        self.connect(self.CW, SIGNAL('stateChanged(int)'),
                     lambda: self.requestSetCW(self.CW.isChecked()))

        labels = ['Power (dBm)', 'CW', 'Center freq.(GHz)', 'Span (GHz)',
                  'Start freq. (GHz)', 'Stop freq. (GHz)', 'IF BW (Hz)', 'Points', 'Avg counts']
        self.widgetNames = ['power', 'CW', 'centerInGHz', 'spanInGHz', 'startInGHz', 'stopInGHz',
                            'BWInHz', 'numOfPoints', 'avgCount']  # same as keys of retruned dictionary
        types = ['double', 'bool', 'double', 'double',
                 'double', 'double', 'double', 'int', 'int']
        self.setNames = ['setTotalPower', 'setCW', 'setCenterInGHz', 'setSpanInGHz',
                         'setStartInGHz', 'setStopInGHz', 'setBandwidth', 'setNumberOfPoints', 'setAverageCount']
        self.getNames = ['totalPower', 'getCW', 'getCenterInGHz', 'getSpanInGHz',
                         'getStartInGHz', 'getStopInGHz', 'bandwidth', 'numberOfPoints', 'averageCount']
        ...
        for label, widgetName in zip(labels, self.widgetNames):
            ...
            if widgetName not in ['CW']:
                subGrid1.addWidget(getattr(self, widgetName), 1, col)
        ...
        for x in range(0, len(self._traces)):
            if self.visibility(x):
                ...
                if self._reference != None and self._reference != self._traces[x]:
                    ...
                    phaseplots.append(
                        self._traces[x]['phase'] - self._reference['phase'])
                else:
    ...
    def updatedGui(self, subject, property=None, value=None, message=None):
        if subject == self.instrument:
            if property == 'getFreqMagPhase':
                ...
                for checkBox, key in zip([self.AvgON, self.CW], ['avgONOFF', 'cwONOFF']):
                    checkBox.blockSignals(True)
                    checkBox.setChecked(value[key])
