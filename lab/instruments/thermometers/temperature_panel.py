...
from gui.mpl.canvas import MyMplCanvas as Canvas
from gui.frontpanel import FrontPanel
from gui.elements.numericedit import *
import pylab
...
class Panel(FrontPanel):
    ...
    def updateTemperature(self, t):
        try:
            ...
            if len(self.gliding) > 1:
                slope = (self.gliding[-1] - self.gliding[1]) / \
                    (self.times[-1] - self.times[1]) * 60 * 60
            else:
                slope = 0
            ...
            if t < 1.0:
                self.temperature.setText("%g mK (%s)%s" % (
                    (t * 1000), slopeInfo, warning))
            else:
                self.temperature.setText("%g K (%s)%s" %
                                         (t, slopeInfo, warning))
    ...
    def __init__(self, instrument, parent=None):
        ...
        self.connect(self.timer, SIGNAL("timeout()"),
                     lambda: self.instrument.dispatch("temperature"))
