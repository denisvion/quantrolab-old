...
class Instr(VisaInstrument):
    ...
    def setOutputValue(self, value, slewrate=None, raiseError=False):
        ...
        if slewrate is None:
            slewrate = self._slewrate
        if slewrate is None:
            self.write('S%f;E;' % value)
        else:
            # and an increment larger than or equal to the precision
            inc = max(abs(slewrate) * 0.1, prec)
            v = self.outputValue()
    ...
    def precision(self):
        ...
        # the exponent
        i2 = string.find('E', i1)
        # calculate the number of digit in the decimal part
        decimals = i2 - i1 - 1
