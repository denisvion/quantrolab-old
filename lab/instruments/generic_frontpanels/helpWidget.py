...
class HelpWidget(QWidget):
    ...
    def myReadGeneral(self):
        ...
        doc = ins.__doc__
        if doc is None:
            doc = 'has no documentation string.'
        attributes = dir(ins)
        methods = [attribute for attribute in attributes if ismethod(
            getattr(ins, attribute))]
        properties = list(set(attributes) - set(methods))
        publicMethods = [method for method in methods if method[0] != '_']
        publicProperties = [
            propert for propert in properties if propert[0] != '_']
        self.generalHelp.clear()
        helpString = clas + ' defined in \n' + mod + ':\n' + doc + '\n'
        helpString += '***********************\nPublic properties\n***********************\n'
        for propert in publicProperties:
            helpString += propert + '\n'
        helpString += '***********************\nPublic methods\n***********************\n'
    ...
    def myMethodHelp(self):
        ...
        if hasattr(ins, selection):
            ...
            if ismethod(attr):
                ...
                if helpString:
                    self.methodHelp.append(QString(helpString))
                else:
                    self.methodHelp.append(
                        QString('Selected method has no documentation string.'))
            else:
                self.methodHelp.append(
                    QString('Selection is not a method name.'))
