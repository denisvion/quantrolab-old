...
class Panel(FrontPanel):
    ...
    def indexOf(self, g):
        for i in range(len(self._generatorsArray)):
            if g == self._generatorsArray[i][1]:
                return i

    def update(self):

        for i in self._generatorsArray:
            i[2].clear()
        ...
        for i in range(len(self._generatorsArray)):
            if i != 0:
                ax = self.plot._fig.add_subplot(
                    len(allPulses), 1, i + 1, sharex=ax)
            else:
                ax = self.plot._fig.add_subplot(len(allPulses), 1, i + 1)
...
#		f, axarr = plt.subplots(len(allPulses), sharex=True)
        for g in allPulses:
            ...
            try:
                ...
                self._generatorsArray[indexInArray][
                    2].insertTopLevelItem(0, item)
                item.itemArray = self._generatorsArray[indexInArray][2]
                addOffset = True
                offset = g.getOffset(0) if addOffset else 0
                item.plot = self.plot._fig.axes[i].plot(
                    g.pulseSequenceArray + offset)
            except:
            ...
            for p in allPulses[g].values():
                ...
                if not(p.pulseOn) and self._showOffPulses:
                    item.plot = self.plot._fig.axes[
                        i].plot(p._pulseArray, '--')
        ...
        plt.setp([a.get_xticklabels()
                  for a in self.plot._fig.axes[:-1]], visible=False)
    ...
    def itemClicked(self, item):
        if item.itemArray.lastSelectedItem is not None:
    ...
    def __init__(self, instrument, parent=None):
        ...
        self.connect(showOffPulsesCommand, SIGNAL(
            "triggered()"), self.showOffPulsesOption)
