...
class Pulse:
    ...
    def __init__(self, name="untitled", pulseGenerator=None, frequency=None, phase=0, generatorFunction="square", applyCorrections=False, pulseOn=True, **kwargs):
        ...
        # made complex by Kiddi 09/03/2015
        self._pulseArray = zeros(self._numberOfPoints, dtype=numpy.complex128)
        self._generatorFunction = generatorFunction
        self._parameters = kwargs

        try:
            self._pulseArray[:] = getattr(generatorFunctionLib, generatorFunction)(
                self._numberOfPoints, self._samplingTime, **kwargs)
...
class Instr(Instrument):
    ...
    def restoreState(self, state):
        ...
        for pulse in state["pulses"].values():
            if pulse["shape"] is None:
                self.generatePulse(duration=pulse["duration"], frequency=pulse["frequency"], gaussian=pulse["gaussian"], amplitude=pulse[
                                   "amplitude"], phase=pulse["phase"], DelayFromZero=pulse["DelayFromZero"], useCalibration=pulse["useCalibration"], name=pulse["name"])
            else:
    ...
    def getOffset(self, i=0):
        ...
        if self._formGeneratorType == "AWG":
            return self._AWG.offset(self._params["AWGChannels"][i])
        if self._formGeneratorType == "AFG":
            return self._AWG.getOffset(self._params["AWGChannels"][i])
        return 0
    ...
    def addMarker(self, channel=None, start1=0, stop1=10000):
        ...
        if channel > self.markersChannels:
            ...
            self.markersList1 += (Marker(pulseGenerator=self,
                                         start1=start1, stop1=stop1),)
        else:
            self.markersList2 += (Marker(pulseGenerator=self,
                                         start1=start1, stop1=stop1),)
    ...
    def sendPulseSequence(self, outputName=None):
        ...
        if self._params['modulationMode'] == "IQMixer":
            ...
            print "self._params['modulationMode'] not correctly set: ", self._params['modulationMode']
    ...
    def sequenceWaveform(self, seqNumber, channel=None, name=None, repeat=1):
        ...
        self._AWG.appendWaveformToSequence(
            seqNumber, channel, name, repeat=repeat)
    ...
    def sequenceWaveformList(self, channel=None, names=None, start=0, stop=0):
        ...
        if names is None:
            ...
            if start == 0 and stop == 0:
                # The first 25 waveforms are the predfined ones
                names = allNames[25:]
            else:
...
# OBSOLETE DON NOT USE ANY MORE. KEPT FOR COMPATIBILITY REASONS
    ...
    # Made by Kiddi 17/09/13
    def generateMarker(self, name=None, start=0, stop=10000, level=3, start2='', stop2='', start3='', stop3=''):
    ...
    # markersName=None added by Kiddi 17/09/2013
    def sendPulse(self, forceSend=False, name=None, markersName=None, outputName=None):
        """
    ...
    def loadWaveformToList(self, waveformName=None, markersName=None, outputName=None):
        ...
        self._AWG.listRealWaveform(
            pulse, markers=markers, waveformName=outputName)
