...
class Instr(Instrument):
    ...
    def calibrateAmplitudeAndOffset(self, f):
        ...
        for phi in arange(0, 2 * math.pi, math.pi / 30):
            ...
            self._pulseGenerator.generatePulse(
                duration=20000, frequency=f, amplitude=0.6, DelayFromZero=0, useCalibration=True, phase=phi)
        ...
        # I0=2/ptp(rowData['I'])
        # Q0=2/ptp(rowData['Q'])
        (I, Q, phi, dphi) = scipy.optimize.fmin_powell(lambda (I, Q, phi0, dphi): sum((I * rowData['I'] - sin(
            rowData['phi'] + phi0 + dphi))**2) + sum((Q * rowData['Q'] - cos(rowData['phi'] + phi0))**2), (1, 1, 0, 0))
        print (I, Q, phi, dphi)
    ...
    def initialize(self, acqiris, acqirisChannels, MWSource, pulse_generator):
        """
...
#    self._acqiris("moduleDLL2.setChannels("+str(acqirisChannels[0])+","+str(acqirisChannels[1])+")")
        ...
        try:
            self._calibration = Datacube()
            self._calibration.setName('analyser IQ mixer Calibration')
            self._calibration.loadtxt(register.parameters()[
                                      '%s Cal' % self._name])
        except:
            pass
        self._Ilist = []
