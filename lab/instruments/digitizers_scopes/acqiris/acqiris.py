...
import sys,getopt,math,os.path,time
...
class Instr(Instrument):
  ...
  def initialize(self,*args,**kwargs): 
    ...
    self._memoryLocked=False      # Flag to forbid the access to the live waveform when it has been passed to the DLL
  ...
  def createDictAndGlobals(self):
    ...
    # the idea is to have this dictionary always up to date and corresponding to the actual state of the instrument
    self._params = dict()
  ...
  def Calibrate(self,option=CAL_FAST,channels=15):
    ...
    """
    self.debugPrint('in acqiris.Calibrate(option=',option,',channels=',channels,')')
  ...
  def configAll(self,set,*args,**kwargs):
    ...
    # Notify only if you accept that your front panel looses its settings at each new configuration   
    return status
  ...
  def AcquireTransfer(self, voltages=True, wantedChannels=15, transferAverage=False, getHorPos=True, getTimeStamps=True,nLoops=1.,timeOut=10):
    ...
    Systematic notification de-activated. Callers have to use a dispatch call to get notification.
    """
    self.debugPrint('in acqiris.AcquireTransfer(voltages=',voltages,',wantedChannels=',wantedChannels,',transferAverage=',transferAverage,',getHorPos=',getHorPos,',getTimeStamps=',getTimeStamps,',nLoops=',nLoops,',timeOut=',timeOut,')')
    #self.nLoops=nLoops
    ...
    self.debugPrint('calling __acqiris_QuantroDLL1.AcquireTransfer() from acqiris.AcquireTransfer() with timeout=',timeOut)
  ...
  def getLastWave(self):
    ...
    if self._memoryLocked: return'memory locked'
    lastWave=dict()
    ...
    lastWave['wave']=self.lastWaveformArray             # Note that the array is not copied, but is passed by reference
    lastWave['timeStampsSize']=self.lastTimeStampsArraySize
    lastWave['timeStamps']=self.lastTimeStampsArray     # Note that the array is not copied, but is passed by reference
    lastWave['horPosSize']=self.lastHorPositionsArraySize
    lastWave['horPos']=self.lastHorPositionsArray       # Note that the array is not copied, but is passed by reference
    lastWave['averageCalculated']=self.lastAverageCalculated     
    lastWave['lastAverageArray']=self.lastAverageArray  # Note that the array is not copied, but is passed by reference                    
    return lastWave
  ...
  def getLastAverage(self,identifier=None,calculate=True):
    ...
    if identifier is not None and identifier != self.lastWaveIdentifier: return None
    if not self.lastAverageCalculated and calculate: self.calculateAverage()
    if not self.lastAverageCalculated: return None
    lastAve=dict()
